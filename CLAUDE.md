...
This project uses AI coding tools, but the final code and manuscript must read as if written by a competent human researcher. Reviewers and editors are increasingly attuned to AI-generated content, and obvious tells will damage credibility. Claude Code must follow these rules strictly.
...
- No comments that reference AI, machine learning, Claude, or any AI tool. No `# Generated by Claude` headers. No `# AI-assisted implementation` notes. The code should be indistinguishable from code written in a text editor.
