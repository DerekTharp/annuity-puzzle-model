...
function emit_grid_convergence()
    ...
    raw_(io, raw"\caption{Grid, Quadrature, and Annuitization-Grid Convergence (Headline Per-Quartile Configuration)}")
    ...
    nl(io, @sprintf(" & \$120 \\times 50\$ & 15 & %.2f & %.4f ", ref[1], ref[2]))
    ...
    raw_(io, raw"\small")
    raw_(io, raw"\item All specifications use baseline parameters ($\gamma = \pGamma$, DFJ bequests, MWR $= \pMwrBaseline$, $\pi = \pInflation$, $\psi = \pPessimism$) under the production per-quartile Social Security assignment, so the production cell matches the headline ownership level. $n_\alpha = \pNAlpha$ in Panels A--C; Panel D varies it.")
