...
#   psi = 0.3   : light friction, well below the BF spending differential
#   psi = 0.5   : approx Blanchett-Finke (2025) spending differential mapping
#   psi = 1.0   : moderate Chalmers-Reuter (2012) range
#   psi = 1.5   : full Chalmers-Reuter (2012) default-vs-opt-in gap mapped naively
#
# Output: tables/csv/psi_sensitivity.csv (one row per psi value)
# Runtime: ~5 full-model solves; ~25 minutes parallel on 32+ vCPU.
...
const PSI_VALUES = [
    ...
    ("UK low (55pp behavioral)",     0.0142),  # Anchor C-low
    ("UK mid (60pp behavioral)",     0.0163),  # Anchor C-mid (production)
    ("UK high (65pp behavioral)",    0.0194),  # Anchor C-high
    ("UK low total (75pp drop)",     0.0281),  # Anchor B-low
...
# (~25 min for the full 11-channel model) instead of N × 25 min serial.
