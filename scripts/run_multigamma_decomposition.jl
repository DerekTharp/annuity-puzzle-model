...
for g in gammas
    ...
    decomp = run_decomposition(
        ...
        # Preference channels: passing the production values so this
        # multi-gamma table reflects the full model. Default kwargs would
...
open(tex_path, "w") do f
    ...
    @printf(f, "\\item All other parameters at production values (DFJ bequests, MWR \$= %.2f\$, \$\\pi = 2\\%%\$, \$\\psi = %.3f\$).\n",
