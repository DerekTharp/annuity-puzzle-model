...
open(tex_path, "w") do f
    ...
    println(f, "medical costs, R-S correlation, pricing loads, the fixed purchase cost, and")
    println(f, "2\\% inflation, but are solved with no pre-existing Social Security or")
    println(f, "defined-benefit income; SPIA ownership rates here therefore differ from the")
    println(f, "headline configuration, which assigns observed SS+DB income by wealth band.")
    println(f, "The exhibit isolates the SPIA--DIA product comparison within a common")
    println(f, "environment. CEV evaluated at good health (H=1).")
