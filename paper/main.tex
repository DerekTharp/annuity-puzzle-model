...
\paragraph{Calibration of \texorpdfstring{$\psi_{\text{purchase}}$}{psi}.} I discipline $\psi_{\text{purchase}}$ via single-moment SMM to UK post-reform evidence from the 2015 pension freedoms---a transported market-design calibration anchor rather than clean identification of a structural behavioral primitive. The reform bundled tax, advice, default, and product-market changes; the empirical signal is best read as an aggregate market-design response. ABI industry sales-volume data \citep{abi2020} record a roughly 75--87\% proportional decline in annuity sales 2014--2016---an aggregate quantity, not a household-level percentage-point shift in disposition. The bracket calibration uses this proportional decline (and the corresponding decline in the ELSA pension-grid disposition records) as a sensitivity target, mapping it through the model into an ownership-rate change. Approximately 15--25 percentage points of the model-implied ownership-rate change is plausibly attributable to the simultaneous removal of a 55\% tax penalty on lump-sum withdrawals (a budget-constraint channel already represented in the model); the residual mapping pins the conservative bracket end at $\psi_{\text{purchase}} = \pPsiBracketLow$. A more aggressive ELSA microdata sensitivity bound, which adopts the full pre/post drop without rational stripping, implies $\psi_{\text{purchase}} = \pPsiBracketHigh$ (Appendix~\ref{A-app:psi_anchors}). The headline calibration bracket $\psi_{\text{purchase}} \in [\pPsiBracketLow, \pPsiBracketHigh]$ corresponds to a predicted US ownership bracket of $[\ownBracketLow, \ownBracketHigh]$. The wider sensitivity interval across alternative single-anchor SMM specifications spans $[0.014, 0.040]$.

\paragraph{Why the rational-corrected anchor.} Three considerations drive the choice of a rational-corrected sensitivity target. First, $\psi_{\text{purchase}}$ in the structural model is a preference-style parameter (loss aversion over a narrowly framed gain/loss tally); the 55\% tax penalty operates through the budget constraint and is already represented in the model's rational pricing channels. Calibrating $\psi_{\text{purchase}}$ to the raw decline would force the parameter to absorb a wedge the structural model handles elsewhere. Second, the UK 2015 reform bundled tax, advice, default, and product-market changes; under any portability assumption, attributing the entire raw decline to a single transported behavioral parameter overstates the cleanliness of the experiment. Third, the rational-corrected anchor preserves interior identification: at $\psi_{\text{purchase}} = \pPsiPurchase$ the model produces a non-corner ownership prediction, while higher anchors (matching the raw UK decline) push the model to a corner where any sufficiently large $\psi_{\text{purchase}}$ produces an identical 0\% prediction.
