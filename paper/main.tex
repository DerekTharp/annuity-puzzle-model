...
\noindent
The order-independent ranking is the paper's central result. Pricing loads are the dominant suppressor of annuity demand (\shapNineLoads{} pp), with subjective survival pessimism (\shapNinePessimism{} pp) and the combined medical-expenditure and Reichling--Smetters health-mortality channel (\shapNineMedRS{} pp) as the co-leading second tier. Bequest motives, the most-cited explanation in the literature, land mid-pack (\shapNineBequests{} pp). Social Security enters with a large negative value ($\shapNineSS$ pp): pre-existing public annuitization is the largest demand-boosting force and raises private demand at the margin by supplying an income floor. The top of the ranking is stable where the level is not. Predicted ownership is knife-edged in risk aversion, moving from \ownGammaSweepLow{} to \ownGammaSweepHigh{} across $\gamma \in [2.0, 3.0]$, yet pricing loads remain the single dominant suppressor at all \gammaStabNGamma{} values of $\gamma$, in every wealth quartile, and under both the discontinuous ownership indicator and the continuous mean-purchase statistic (suppressor rank correlation \gammaStabSpearmanSup). The lower tiers reshuffle with risk aversion---bequest motives, in particular, are never the leading channel but rise from mid-pack at the baseline toward the second tier at low $\gamma$---so the robust claim is that pricing dominates and bequests do not, not a fixed full ordering.

\noindent
The exercise reframes the puzzle as distributional. Predicted ownership concentrates entirely in the top wealth quartile (\ownFullQfour, versus \ownFullQone{} below it). The bottom half is rationally excluded by pre-existing annuitization, the minimum-purchase floor, and precautionary liquidity held against the Medicaid consumption floor. A 22\% Social Security cut, approximating the projected OASI shortfall on trust-fund depletion in late 2032, raises private demand only among the wealthy who least need the substitution, while the households the cut most exposes cannot respond at all. In HRS cross-sections the resource-side channels are sign-consistent with the patterns they predict (\empSignMatch{} of \empSignTotal{} predicted signs, though individually imprecise). At the baseline calibration the disciplined nine-channel model predicts \ownNineChannelLTC{} ownership, but this is a point on a steep surface---across the defensible money's-worth-ratio range the same model spans roughly \ownMWREightyFive{} to \ownMWRNinety{}, bracketing the two HRS measures (\pctHRSLifetime{} lifetime-contract, \pctHRSIannPooled{} any-annuity proxy)---so the ranking, not the level, is the paper's headline. An exploratory extension then probes a question the structural annuity literature has left almost untouched: at literature-plausible magnitudes, two un-identified behavioral channels---not moment-matched, and held off in every headline result---each move predicted ownership by more than any structural channel. Because they offset and one saturates the model, they are reported not as ranked magnitudes but as evidence that an almost-unexplored behavioral frontier could outweigh the forces the literature does study.
...
Varying $\psi$ from 0.970 to 1.000 (objective beliefs) shifts predicted ownership from \ownPsiNinetySeven{} to \ownPsiObjective. At the baseline $\psi = \pPessimism$ the model predicts \ownPsiBaseline; at $\psi = 0.981$---the per-year factor implied directly by the \citet{odeasturrock2023} ten-year survival ratio $(0.71/0.86)^{1/10}$, a less severe and more defensible calibration than the baseline---it predicts \ownPsiNinetyEightOne{}, inside the model's wide money's-worth and risk-aversion band. The steep sensitivity confirms that survival pessimism is a quantitatively meaningful channel, consistent with its Shapley value of \shapNinePessimism{} pp at the baseline; the level it implies, like the rest of the model's level predictions, is calibration-sensitive while the channel's second-tier rank is not.
