...
The annuitization fraction $\alpha$ is optimized over a \pNAlpha-point grid at age 65. Grid convergence is verified at the \pNQuad-node production quadrature rule: on the mean-Social-Security diagnostic, predicted ownership ranges from 21.1\% at the medium grid ($60 \times 20$) to 20.8\% at the fine grid ($100 \times 40$), bracketing the production grid ($80 \times 30$, 20.0\%); the roughly 1 pp variation across grid resolutions is small relative to the channel contributions in the Shapley decomposition. See Appendix Table~\ref{A-tab:grid_convergence}.
