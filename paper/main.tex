...
Both HRS measures are reported as out-of-sample comparators for Model 1 (\ownNineChannelLTC{} nine-channel structural baseline) and Model 2 (bracket $[\ownBracketLow, \ownBracketHigh]$). The conventional income proxy sits just below the Model 2 bracket's lower edge with its upper 95\% confidence bound extending into the bracket; the cleaner lifetime-contract indicator falls below the bracket. Section~\ref{sec:results} reports headline results against both measures.
...
The full eight-channel rational+preference model therefore predicts \ownNineChannelFLN{} ownership under the FLN central mapping and \ownNineChannelRS{} under the Reichling--Smetters translation, a reduction from the \ownFrictionless{} frictionless benchmark but still above both empirical targets ($\pctHRSIannPooled$ on the conventional any-annuity income proxy; $\pctHRSLifetime$ on the cleaner lifetime contract indicator). Adding the structural public-care aversion channel ($\chi_{\text{LTC}} = \pChiLTC$) yields the nine-channel structural baseline of \ownNineChannelLTC, modestly above both HRS comparators. The remainder of the paper uses the FLN mapping as the production calibration.

\subsection{Behavioral extensions and the Model 2 transport}
\label{sec:behavioral_results}

The eight-channel rational+preference model predicts \ownEightChannelExt{} ownership. Adding the structural public-care aversion channel ($\chi_{\text{LTC}} = \pChiLTC$) yields the nine-channel structural baseline of \ownNineChannelLTC{}, modestly above the two HRS empirical comparators ($\pctHRSLifetime$ on the lifetime contract indicator, $\pctHRSIannPooled$ on the conventional any-annuity income proxy). The nine-channel baseline brings predicted ownership into the same neighborhood as the HRS measures using structural mechanisms alone, overshooting the conventional income proxy by roughly 1.6 pp and the lifetime indicator by roughly 3.7 pp. This is the disciplined Model 1 reading. Notably, Model 2's UK reduced-form transport (Section~\ref{sec:behavioral}) places the production midpoint at \ownHeadline{} (bracket $[\ownBracketLow, \ownBracketHigh]$), within 1 percentage point of the structural baseline---two independent identification strategies converging on essentially the same value.
...
\paragraph{Out-of-sample comparison.} The conventional any-annuity income proxy ($\pctHRSIannPooled$, Wilson 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$) and the cleaner lifetime-contract indicator ($\pctHRSLifetime$, Wilson 95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$) are reported as out-of-sample comparisons against both the Model 1 nine-channel baseline (\ownNineChannelLTC) and the Model 2 bracket ($[\ownBracketLow, \ownBracketHigh]$). The conventional income proxy sits just below the Model 2 bracket's lower edge with its upper 95\% confidence bound extending into the bracket; the lifetime-contract indicator falls below the bracket. The two models reach the same neighborhood from independent directions, providing cross-validation rather than a single moment-matched fit.
...
\paragraph{Nine-channel structural Shapley (headline).} Table~\ref{tab:shapley_nine} reports the disciplined attribution. Among demand-suppressing channels, pricing loads (\shapNineLoads{} pp, \shapShareNineLoads) and the combined Med+R-S health-mortality channel (\shapNineMedRS{} pp, \shapShareNineMedRS) are the two largest contributors, followed by age-varying consumption needs (\shapNineAgeNeeds{} pp, \shapShareNineAgeNeeds), survival pessimism (\shapNinePessimism{} pp, \shapShareNinePessimism), and inflation erosion (\shapNineInflation{} pp, \shapShareNineInflation). Bequest motives (\shapNineBequests{} pp), state-dependent utility (\shapNineStateUtil{} pp), and public-care aversion (\shapNineLTC{} pp) contribute small magnitudes. Social Security enters with a negative Shapley value ($\shapNineSS$ pp, $\shapShareNineSS$), reflecting its role as a complement at the structural baseline: SS provides the income floor that makes additional annuitization rational at the margin, so its marginal contribution to the demand-suppression total is negative.
...
Under the baseline calibration ($\gamma = \pGamma$, MWR $= \pMwrBaseline$), the six standard rational channels predict \ownSixChannel{} ownership relative to a frictionless population benchmark of \ownFrictionless{}---above both empirical targets and echoing the residual overprediction in \citet{pashchenko2013}. The two preference channels bring the prediction to \ownEightChannelExt{}, and adding the structural public-care aversion channel yields the nine-channel Model 1 baseline of \ownNineChannelLTC{}, modestly above the two HRS empirical comparators ($\pctHRSIannPooled$ on the conventional any-annuity income proxy, 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$; $\pctHRSLifetime$ on the cleaner lifetime-contract indicator, 95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$). Model 2's UK reduced-form transport against the frictionless benchmark of \ownFrictionless{} delivers a complementary bracket of $[\ownBracketLow, \ownBracketHigh]$ with midpoint \ownHeadline{}: the conventional HRS income proxy sits just below the bracket's lower edge (with its upper 95\% confidence bound extending into the bracket) and the lifetime-contract indicator falls below the bracket. The two independent identification strategies converge within roughly 1 percentage point of each other; both lie modestly above the HRS empirical comparators. The disciplined reading is the parallel report of Model 1 and Model 2, not a single moment-matched figure.
