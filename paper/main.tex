...
\begin{abstract}
\noindent
Why is voluntary annuity ownership only a few percent when \citet{yaari1965} implies full annuitization? Competing explanations abound, but the sequential decompositions used to weigh them are order-dependent. We nest nine rational and preference channels in a calibrated lifecycle model of single US retirees and attribute the demand gap with exact Shapley values over all $2^{9} = 512$ channel subsets. Pricing loads dominate (\shapNineLoads{} pp), with survival pessimism (\shapNinePessimism{} pp) and correlated health--mortality risk (\shapNineMedRS{} pp) in the second tier; bequest motives, the most-cited explanation, are only mid-pack (\shapNineBequests{} pp), while pre-existing annuitized income raises demand on average rather than suppressing it. This ranking is stable even though the predicted level is not: ownership swings from \ownGammaSweepLow{} to \ownGammaSweepHigh{} across plausible risk aversion, bracketing the \pctHRSLifetime{}--\pctHRSIannPooled{} observed in the Health and Retirement Study, yet pricing loads lead at every value and in every wealth band. The decomposition also reframes the puzzle as distributional, a question sharpened by the projected depletion of the Social Security trust fund around 2032. The same pricing load is the binding margin across the wealth distribution: at the baseline money's-worth ratio, immediate annuitization is value-destroying for all but the wealthiest, so predicted ownership rises with wealth and concentrates at the top. Observed lifetime-annuity ownership also rises with wealth but is interior in the middle of the distribution, where the single-product model predicts none---a gap we attribute to better-priced employer and group annuities the model omits. A 22\% benefit cut therefore raises private annuitization among the wealthy who least need the substitution while barely moving the lower-wealth households the cut most exposes: private markets do not backfill a public benefit cut where it bites hardest, and the lever that would is group or public-option pricing, not privatized longevity insurance.
