...
% Defensive placeholders for macros introduced under Option 1 bundled
% identification. \providecommand only defines the macro if it isn't already
% defined in numbers.tex, so the manuscript compiles cleanly during the
% transition period before the pipeline regenerates numbers.tex with the new
% multiplicative-wedge macros. Once the AWS rerun completes, numbers.tex
% emits these macros directly and these placeholders silently no-op.
\providecommand{\ownNoBehavioralBaseline}{[pending]}
\providecommand{\ownWedgeLow}{[pending]}
\providecommand{\ownWedgeMid}{[pending]}
\providecommand{\ownWedgeHigh}{[pending]}
\providecommand{\ownHeadline}{[pending]}
\providecommand{\pWedgeFactorLow}{0.137}
\providecommand{\pWedgeFactorMid}{0.179}
\providecommand{\pWedgeFactorHigh}{0.263}
\providecommand{\pUKRetentionPre}{95\%}
\providecommand{\pUKRetentionLow}{13\%}
\providecommand{\pUKRetentionMid}{17\%}
\providecommand{\pUKRetentionHigh}{25\%}
\providecommand{\pChiLTC}{0.7}
% Legacy macros (retained for backward-compatibility prose; will be regenerated
% by the new export script as either values or "[obsolete]" placeholders).
\providecommand{\pLambdaW}{[obsolete]}
\providecommand{\pPsiPurchase}{[obsolete]}
\providecommand{\pPsiPurchaseCRef}{18{,}000}
\providecommand{\pPsiBracketLow}{[obsolete]}
\providecommand{\pPsiBracketHigh}{[obsolete]}
\providecommand{\ownTenChannelFull}{\ownNoBehavioralBaseline}
\providecommand{\ownTenChannelFullNum}{\ownNoBehavioralBaseline}
\providecommand{\ownElevenChannelFull}{\ownHeadline}
\providecommand{\ownNineChannelSDU}{\ownNoBehavioralBaseline}
\providecommand{\shapSDU}{0.0}
\providecommand{\shapShareSDU}{0\%}
\providecommand{\shapNarrowFraming}{[N/A]}
\providecommand{\shapShareNarrowFraming}{[N/A]}
\providecommand{\shapBundledWedge}{[N/A]}

\externaldocument[A-][nocite]{appendix}

% ---- Title ----
\title{Decomposing the Annuity Puzzle: \\
Rational, Preference, and Behavioral Channels under \\
a UK-Anchored Wedge Transport\thanks{%
...
\begin{abstract}
\noindent
\citet{yaari1965} showed that a consumer with uncertain lifetime and no bequest motive should fully annuitize under actuarially fair pricing. Observed voluntary annuity ownership among US retirees is only a few percent. I build a lifecycle model that begins from a standard rational and preference-based account of annuity demand and then adds a \emph{bundled behavioral wedge} representing three conceptually distinct phenomena that activate when compulsion lifts: source-dependent utility (SDU; \citealp{blanchett2024,blanchett2025}); narrow-framing purchase-event disutility (PED; \citealp{barberishuang2009,tverskykahneman1992}); and choice-architecture salience (provider-communication, adviser-conversation, status-quo, and default-vs-opt-in framing effects; \citealp{madrian2001, brown2008framing}). The bundle is identified externally from a single market-design moment---the UK 2015 pension-freedoms reform---and applied to the model's no-behavioral baseline as a deterministic multiplicative transformation: predicted US ownership equals the no-behavioral baseline multiplied by the UK proportional retention factor (post/pre $\approx 17/95 = 0.18$ at the production midpoint). The three named components are not separately parameterized in the model; they are observed jointly in UK 2015 and transported jointly. Without the wedge the model substantially overshoots observed US ownership; applying the wedge to the no-behavioral baseline yields the production prediction at the production midpoint, with a sensitivity range across UK retention anchors that contains both HRS empirical measures.
...
\noindent
The bundled wedge is implemented as multiplicative post-processing on ownership rates rather than as a structural parameter inside the Bellman equation: the model produces the no-behavioral baseline (rational + preferences + the structural public-care aversion channel $\chi_{\text{LTC}}$), and the wedge is applied externally as a deterministic scalar reduction. The Blanchett--Finke spending differential is descriptive evidence that source-dependent utility exists in retirement spending data, but no separate identifying moment is used; the bundle is identified jointly from UK 2015 alone. Counterfactuals that ``remove the bundled wedge'' are interpreted as removing the multiplicative reduction, not as removing a single named force. Channel attribution among the model-internal channels uses exact Shapley values computed over all $2^9 = 512$ subsets of the nine model-internal channels; the bundled wedge appears separately as a multiplicative reduction, not as a Shapley channel.
...
The paper then adds a \emph{bundled behavioral wedge} representing three named phenomena that activate when compulsion lifts. Source-dependent utility (SDU) captures the post-purchase ``license to spend'' that retirees apply to guaranteed income flows but not portfolio assets \citep{shefrin1988, thaler1999, blanchett2024, blanchett2025}. Narrow-framing purchase-event disutility (PED) captures the at-purchase loss aversion over the unrecouped premium until breakeven \citep{tverskykahneman1992, barberishuang2009, brown2008framing, chalmersreuter2012}. Choice-architecture salience captures the sub-bundle of provider-communication framing, adviser-conversation effects, status-quo bias, and Madrian--Beshears default-vs-opt-in inertia \citep{madrian2001, goda2014}. The bundle is identified from a single external moment---the UK 2015 pension-freedoms reform---under a \emph{proportional behavioral wedge} transport that treats the post/pre retention ratio as country-invariant. The wedge is applied to the model's no-behavioral baseline as a deterministic multiplicative reduction; no behavioral parameter is calibrated inside the Bellman equation. The 2015 reform bundled tax, advice, default, and product-market changes; the empirical evidence is best read as a market-design anchor rather than a clean causal estimate of any single behavioral primitive in isolation. The English Longitudinal Study of Ageing (ELSA) pension grid provides supporting individual-level evidence: $\pctELSAWaveSixAnnuity$ of wave 6 (2012--13) DC pension recipients reported annuity-style income under the pre-freedoms mandate, and pooling waves 8--11 (2016--24), $\pctELSAPostAnnuity$ of observed lump-sum disposition records used the lump sum to buy annuity income (n=\nELSAPostLumpSum). No US moment is used to discipline the bundle; consistency with US ownership is an out-of-sample comparison.

The paper's contribution is fourfold. First, the unified framework---the first to nest age-varying consumption needs alongside the established rational and preference channels of the annuity-puzzle literature. Second, the formal taxonomy of three named behavioral phenomena (SDU, PED, choice-architecture salience) treated as a single bundled wedge identified from a single external moment. Third, the proportional-wedge transport from the UK 2015 pension-freedoms reform as a disciplined identifying assumption, with explicit cross-country preference-homogeneity assumption. Fourth, an exact Shapley decomposition over all $2^9 = 512$ subsets of the nine model-internal channels that attributes the demand reduction without order dependence.
...
The exact Shapley decomposition over the nine model-internal channels is the paper's preferred attribution result for the no-behavioral baseline. Among demand-suppressing model channels, pricing loads (\shapLoads{} pp, \shapShareLoads) and the combined medical risk and Reichling--Smetters health-mortality correlation channel (\shapMedRS{} pp, \shapShareMedRS) are the two largest contributors, followed by age-varying consumption needs (\shapAgeNeeds{} pp, \shapShareAgeNeeds), survival pessimism (\shapPessimism{} pp, \shapSharePessimism), and inflation erosion (\shapInflation{} pp, \shapShareInflation). Social Security enters with a negative Shapley value ($\shapSS$ pp, $\shapShareSS$), reflecting its role as an ownership booster. The bundled behavioral wedge does not appear in the Shapley enumeration: it is applied as multiplicative post-processing on the ownership rate produced by the no-behavioral baseline. State-dependent utility and bequest motives both contribute negligibly (\shapStateUtil{} pp and \shapBequests{} pp respectively), entering the paper as completeness checks rather than main drivers.

The policy and welfare results extend beyond the ownership match. Group annuity pricing (MWR $= 0.90$) raises predicted ownership relative to the production calibration. Removing the bundled behavioral wedge (an illustrative default-architecture lever) recovers the no-behavioral baseline of \ownNoBehavioralBaseline{}---a counterfactual roughly consistent with the UK 2015 reform and the \citet{goda2014} federal Thrift Savings Plan natural experiment, but reported with the explicit caveat that the wedge is identified only as a bundle and cannot be decomposed into the contributions of its named components. Social Security cut counterfactuals reveal that a 23\% benefit cut---the projected consequence of trust fund exhaustion around 2033---raises private annuity demand substantially, but the largest cuts eventually reduce demand by removing the income floor that makes private annuitization feasible at all. Under current pricing the welfare gain from annuity market access is small across the wealth distribution because most calibrated households optimally do not annuitize, so access is approximately neutral; policy interventions that change the optimal choice (group pricing, demand-side architecture) generate the larger welfare gains.
...
I treat all three forces as components of a single \emph{bundled behavioral wedge} that is jointly identified from one external moment---the UK 2015 pension-freedoms reform. The reform turned Force A, Force B, and Force C on simultaneously when compulsion lifted: source-dependence, narrow framing, and choice-architecture salience all became binding at the same moment when retirees were given a free choice. The resulting attrition is the joint footprint of the three forces, which is what the data permit transporting.

\paragraph{Mechanism: multiplicative transport.} The bundled wedge is applied as a deterministic multiplicative transformation to the model's no-behavioral baseline. Under proportional-wedge identification, the ratio of post-reform UK voluntary retention to pre-reform UK compulsory retention is treated as country-invariant:
\begin{equation}
\label{eq:wedge}
\text{predicted US ownership} = \text{no-behavioral US baseline} \times \frac{\text{UK retention}_{\text{post}}}{\text{UK retention}_{\text{pre}}}.
\end{equation}
Forces A, B, and C are NOT parameterized in the model itself. They are named in the manuscript as conceptually distinct components of the bundle (Section~\ref{sec:behavioral_intuition}), but no single force is separately mechanized. The model's role is to produce the no-behavioral baseline (rational + preferences + structural channels including $\chi_{\text{LTC}}$); the wedge is applied as post-processing in scripts/export\_manuscript\_numbers.jl.

The proportional retention factor at the production midpoint is $17/95 \approx 0.18$. The UK retention sensitivity range of [13\%, 25\%] maps to factors of [0.137, 0.263]. Applied to the no-behavioral baseline of \ownNoBehavioralBaseline, the wedge produces a predicted US ownership bracket of [\ownWedgeLow, \ownWedgeHigh] with production midpoint \ownWedgeMid.

\paragraph{Why mechanism-free.} The model could in principle parameterize each behavioral channel structurally---an SDU consumption transformation $c_{\text{eff}} = c_{\text{income}} + \lambda_W c_{\text{portfolio}}$ for Force A, an at-purchase loss-aversion penalty $\psi_{\text{purchase}}$ for Force B---and calibrate them via SMM bisection. We considered and rejected this approach for two reasons. First, two parameters cannot be separately identified from a single bundling moment without auxiliary anchors that re-introduce the multi-moment structure that bundled identification rejects. Second, the at-purchase penalty mechanism produces non-smooth participation thresholds at low pre-behavioral baselines (where $\chi_{\text{LTC}}$ leaves little room for the wedge to operate); the threshold behavior does not faithfully implement the multiplicative transport interpretation. Multiplicative transport as post-processing is the cleanest implementation of proportional-wedge identification.

\paragraph{Cross-country transport: maintained assumption.} Proportional-wedge identification is a transport assumption, not a property of the data. The maintained assumption is that the equilibrium retention ratio (post-reform / pre-reform) is country-invariant: UK and US retirees have sufficiently similar preference distributions over the SDU, PED, and choice-architecture-salience phenomena that the same proportional reduction applies in both populations. This assumption is load-bearing. Specifically: (i) the cross-country preference homogeneity of the underlying behavioral phenomena, (ii) sample exchangeability between UK 2015 DC pension-pot decumulators and US 65--69 retirees in the HRS sample, and (iii) institutional comparability sufficient that the same retention ratio applies. UK and US institutional environments differ along several dimensions not absorbed by the wedge: DC pension-pot scale and concentration, the FCA/RDR adviser regime versus US insurance-distribution structures, Medicaid/SS interaction patterns, and minimum-purchase requirements. The wedge bracket [\ownWedgeLow, \ownWedgeHigh] is therefore an out-of-sample extrapolation under a transport assumption, not a clean identification result.

\paragraph{Tax-penalty and rate-environment confounds.} Two confounds within the UK identifying moment itself are worth flagging. First, the UK 55\% unauthorised-payment charge that enforced pre-reform compulsion is a budget-constraint mechanism not a preference parameter; the proportional-wedge identification absorbs this into the bundle but the implied ``behavioral'' attrition therefore includes a UK-specific tax-removal response (estimated at 5--15 pp of the raw 75--87 pp swing; see \citealp{cribb2018annuity, cannon2016annuity} for analyses of the UK reform's tax and behavioral components). Second, the rate-environment confound is empirically small: FCA Retirement Income Market Data \citep{fca2025} report annuity share of DC pots accessed at approximately 9\% in 2024/25 despite gilt yields at 14-year highs, indicating that the post-2015 retention regime has been broadly stable across a full rate cycle and that gilt-yield variation explains 0--5 pp at most of the post-reform retention level. The wider sensitivity range covers both confounds.

\paragraph{Out-of-sample comparison.} The calibration is set entirely from UK data, with no reference to observed US ownership. The predicted US voluntary annuitization bracket of [\ownWedgeLow, \ownWedgeHigh] is therefore an out-of-sample comparison. Both HRS measures of US lifetime annuity ownership ($\pctHRSLifetime$ and $\pctHRSIannPooled$) are reported alongside the prediction in Section~\ref{sec:results}.

\paragraph{Conceptually distinct margins, jointly identified.}\label{sec:behavioral_intuition} Within the bundle, the three named forces target empirically distinct margins. Source-dependent utility (Force A) operates on the \emph{ongoing consumption} from existing wealth (post-purchase, every period); it shows up in passive spending patterns documented by Blanchett--Finke and in the fungibility violations documented by \citet{hastings2013}. The narrow-framing penalty (Force B) operates on the \emph{at-purchase decision} (one-time, with stream-form ongoing pain until breakeven); it shows up at transaction boundaries documented by Brown et al.\ and in the cumulative-prospect-theory mechanism formalized by \citet{huscott2007}. Choice-architecture salience (Force C) operates on the \emph{decision environment surrounding the purchase} (provider communications, adviser scripts, default-vs-opt-in framing); it shows up in the UK 2015 quasi-experiment, the TSP defaults \citep{goda2014}, and the broader Madrian--Beshears defaults literature \citep{madrian2001}. The literatures are distinct, but the UK identifying moment activated all three at once and cannot, on its own, attribute the swing to any one of them. Sub-decomposing the wedge into Force A / Force B / Force C contributions is therefore not pursued.
...
The bundled behavioral wedge does not enter equation~(\ref{eq:multiplicative_R}) as a smooth multiplicative factor on $R$. The wedge is applied externally as a multiplicative reduction on predicted ownership (equation~\ref{eq:wedge}), not as a value-function adjustment. The marginal-margin framework above therefore characterizes the model's no-behavioral baseline; the wedge then reduces predicted ownership proportionally without altering the agent's participation-threshold computation. The Shapley decomposition in Section~\ref{sec:shapley}, which is computed on model-internal channels only, attributes the channels in the no-behavioral baseline.
...
Public-care aversion & $\chi_{\text{LTC}}$ & \pChiLTC & \citet{ameriks2020} \\[4pt]
\multicolumn{4}{l}{\textit{Bundled behavioral wedge (multiplicative transport)}} \\
...
\paragraph{Bundled behavioral wedge (multiplicative transport).} The bundled wedge---containing Force A (SDU), Force B (narrow-framing PED), and Force C (choice-architecture salience) as named conceptual components---is identified externally from the UK 2015 pension-freedoms reform and applied to the model's no-behavioral baseline as a deterministic multiplicative transformation (Section~\ref{sec:behavioral}, equation~\ref{eq:wedge}). The proportional retention factor at the production midpoint is $\rho = \pWedgeFactorMid$ (UK post-reform retention 17\% divided by pre-reform compulsory retention 95\%). The UK retention sensitivity range of [\pUKRetentionLow, \pUKRetentionHigh] maps to a US prediction bracket of [\ownWedgeLow, \ownWedgeHigh] when applied to the no-behavioral baseline of \ownNoBehavioralBaseline. The wedge is NOT a model parameter: the SDU consumption transformation and the at-purchase penalty mechanism have been removed from the model under Option 1 bundled identification. Forces A, B, and C are conceptually named but not separately parameterized; they are observed jointly in UK 2015 and transported jointly. The English Longitudinal Study of Ageing (ELSA) pension grid provides supporting individual-level evidence: in wave 6 (2012--13, pre-freedoms), $\pctELSAWaveSixAnnuity$ of DC pension recipients reported annuity-style income under the mandate; pooling waves 8--11 (2016--24, post-freedoms), $\pctELSAPostAnnuity$ of observed lump-sum disposition records used the lump sum to buy annuity income (n=\nELSAPostLumpSum).
...
\paragraph{The bundled behavioral wedge.} The bundled wedge is applied to the model's no-behavioral baseline as a deterministic multiplicative transformation (Section~\ref{sec:behavioral}, equation~\ref{eq:wedge}): predicted US ownership equals the no-behavioral baseline multiplied by the UK proportional retention factor $\rho = $ post/pre. At the production midpoint ($\rho = 17/95 \approx 0.18$), applying the wedge to the no-behavioral baseline of \ownNoBehavioralBaseline{} yields a production headline prediction of \ownHeadline. The wedge is post-processing on ownership rates; no behavioral parameter is calibrated inside the Bellman equation.

\paragraph{Sensitivity bracket from UK retention range.} Uncertainty in the UK post-reform retention rate (13--25\% across ABI/FCA estimates) maps directly to a US prediction bracket of [\ownWedgeLow, \ownWedgeHigh]. A wider sensitivity range across alternative anchors, including the descriptive ELSA microdata total drop, spans approximately [2.5\%, 8.8\%]. The bracket reflects uncertainty in the UK identifying moment, not parameter calibration uncertainty inside the model.

\paragraph{Bracket against the empirical targets.} The conventional any-annuity income proxy ($\pctHRSIannPooled$, Wilson 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$) and the cleaner lifetime-contract indicator ($\pctHRSLifetime$, Wilson 95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$) are reported as out-of-sample comparisons against the wedge bracket; neither bracket bound is tuned to a US moment.

\paragraph{Decomposition within the bundle is not data-identified.} The bracket reflects the joint footprint of Forces A, B, and C; the data permit transporting only the bundle, not the individual channel contributions. Counterfactuals that ``remove a single force'' are therefore reported in Section~\ref{sec:counterfactuals} as illustrative bundle-removal exercises, not as separately-identified channel attributions. Under joint identification (Section~\ref{sec:behavioral}), sub-decomposing the wedge into Force A / Force B / Force C contributions is parameter-dependent rather than data-identified.
...
Table~\ref{tab:shapley_exact} reports the exact order-independent attribution results for the nine model-internal channels (the bundled behavioral wedge is applied separately as multiplicative post-processing and does not appear in the Shapley enumeration). Among demand-suppressing channels, pricing loads (\shapLoads{} pp, \shapShareLoads) and the combined Med+R-S health-mortality channel (\shapMedRS{} pp, \shapShareMedRS) are the two largest contributors, followed by age-varying consumption needs (\shapAgeNeeds{} pp, \shapShareAgeNeeds), survival pessimism (\shapPessimism{} pp, \shapSharePessimism), and inflation erosion (\shapInflation{} pp, \shapShareInflation). Bequest motives (\shapBequests{} pp) and state-dependent utility (\shapStateUtil{} pp) are quantitatively negligible.
...
The bundled behavioral wedge does not appear in the Shapley enumeration. Under Option 1 bundled identification, the wedge is applied as multiplicative post-processing on ownership rates (equation~\ref{eq:wedge}), not as a model channel: the Shapley enumeration covers only the model-internal nine channels listed above. The wedge's contribution is therefore reported separately as the multiplicative reduction from \ownNoBehavioralBaseline{} (the nine-channel no-behavioral baseline) to \ownHeadline{} (production midpoint after wedge), with bracket [\ownWedgeLow, \ownWedgeHigh].
...
Table~\ref{tab:pairwise} reports the pairwise interaction strength for the eight rational and preference channels. For each pair (A, B), the interaction is defined as the difference between the ownership drop when both are active simultaneously and the sum of their individual drops. A negative interaction indicates super-additivity: the channels reinforce each other. The bundled behavioral wedge is omitted from this table because it is applied as multiplicative post-processing on the no-behavioral baseline rather than as a model channel; the order-independent contributions of the model-internal channels are reported in the exact Shapley decomposition (Section~\ref{sec:shapley}).
...
At the baseline $\gamma = \pGamma$, the six-channel rational model predicts \ownSixChannel{}. Predicted ownership rises with $\gamma$: \ownGammaTwo{} at $\gamma = 2.0$, \ownGammaTwoPointFour{} at $\gamma = 2.4$, and \ownGammaThree{} at $\gamma = 3.0$. The eight-channel rational+preference model (with age-varying needs and state-dependent utility) predicts \ownEightChannelExt{} at the baseline $\gamma = \pGamma$, still substantially above both empirical targets. Adding the structural public-care aversion channel ($\chi_{\text{LTC}}$) yields the no-behavioral baseline of \ownNoBehavioralBaseline{}; applying the bundled behavioral wedge as multiplicative transport (Section~\ref{sec:behavioral}) yields the production headline prediction of \ownHeadline{} at the UK production midpoint with bracket [\ownWedgeLow, \ownWedgeHigh] across UK retention anchors.
...
The economic content of these small CEVs is the same as the low predicted ownership rate: most households would not optimally purchase an annuity at current prices, so giving them access to the market provides little marginal value. The CEV grid is therefore the welfare counterpart of the ownership prediction---both confirm that the marginal household is approximately indifferent at the no-purchase margin. Larger welfare gains appear only when policy changes the optimal choice: lowering the load (group pricing) or removing the bundled behavioral wedge (default architecture), reported in Section~\ref{sec:counterfactuals}.

Section~\ref{sec:counterfactuals} examines how supply-side reforms change these welfare results.

\paragraph{Normative interpretation of the behavioral wedge.} The bundled wedge does not enter the within-model welfare computation directly: under Option 1 bundled identification, the wedge is applied as multiplicative post-processing on ownership rates rather than as a structural parameter affecting agent decisions. The agent's CRRA value-ratio CEV is therefore computed on the no-behavioral baseline. A default-architecture intervention that ``removes the wedge'' has a normatively contingent welfare interpretation \citep{bernheimrangel2009, beshears2008}: under a preference interpretation of the underlying SDU/PED/salience phenomena (welfare-relevant), removing the wedge does not increase agent welfare because the agents' own preferences include those phenomena; under a bias interpretation (welfare-irrelevant), removing the wedge captures the full rational-utility gain. The ranking of policy interventions across the bracket is preserved across both interpretations, but the magnitude of the demand-side default-architecture gain depends on which interpretation is adopted. The empirical exercise in this paper does not adjudicate the preference-vs-bias choice; welfare claims are reported with this caveat made explicit.
...
\paragraph{Best feasible (supply-side + information) package.} For ownership counterfactuals, the ``best feasible'' scenario combines group pricing (MWR $= 0.90$), inflation protection (real annuity), and corrected survival beliefs ($\psi = 1.0$), producing \ownBestFeasible{} ownership at the no-behavioral baseline. This is the upper bound of the supply-side and information interventions on the model-internal channels, not a realistic forecast of market participation. The ``best feasible + default architecture'' scenario in the welfare counterfactuals (Section~\ref{sec:welfare}) below additionally removes the bundled behavioral wedge, recovering the full no-behavioral baseline; the two are tracked separately because removing the wedge has a normatively contingent welfare interpretation (preference vs.\ bias).

\paragraph{Demand-side architecture.} The bundled behavioral wedge admits a distinct policy lever that supply-side reforms cannot replicate: changing the choice architecture so that annuitization is the default rather than an opt-in decision. Removing the wedge under the production calibration (i.e., reporting the no-behavioral baseline directly without applying the multiplicative reduction) raises predicted ownership to \ownNoBehavioralBaseline{}. Under bundled identification, this counterfactual is interpreted as removing the entire bundled wedge---the joint footprint of source-dependent utility, narrow framing, and choice-architecture salience that the UK reform reveals when compulsion lifts---rather than removing one named force. The counterfactual is well-defined as an aggregate wedge-removal intervention; sub-decomposing it into separate SDU / PED / salience contributions is not data-identified. The UK 2015 pension-freedoms reform---the calibration anchor for the bundle---produced a 75--87 percentage-point drop in DC-pot annuity ownership when regulatory compulsion was removed (the pre-reform regime enforced annuitization through a 55\% tax penalty on alternative withdrawals; the post-reform regime is opt-in with marginal-rate tax neutrality) \citep{abi2020}; the \citet{goda2014} federal Thrift Savings Plan annuity option records an $\approx$86 pp household-level default-vs-opt-in gap, a quasi-experimental signal that supports the order of magnitude of the bundled wedge effect. Demand-side architecture is quantitatively comparable to the largest supply-side reforms considered here, while requiring no change to insurer pricing or product design.

\input{../tables/tex/cev_counterfactuals.tex}

\paragraph{Welfare under reform.} Under baseline pricing, the welfare gain from market access is small at every wealth-health cell---\cevGoodTwoHundredKBaseline{} for Good health with DFJ bequests at \$200{,}000---because most calibrated households would not optimally annuitize at current prices. Policy interventions that change the optimal choice generate the larger welfare gains. Group pricing (MWR $= 0.90$) roughly quadruples CEV at moderate wealth: \cevGoodTwoHundredKGroupPrice{} at \$200{,}000 (Good health, DFJ). The ``best feasible'' scenario (group pricing combined with default architecture, which removes the bundled behavioral wedge) extends gains further: \cevGoodHundredKBestFeas{} at \$100{,}000 and \cevGoodTwoHundredKBestFeas{} at \$200{,}000. CEV is near zero at high wealth under baseline, group-pricing-only, and real-annuity-only scenarios. The large high-wealth gains in the best-feasible column arise only when the default-architecture counterfactual removes the bundled wedge, so that column should be interpreted as a behavioral-choice-architecture lever rather than a price-only reform. The values reported in the welfare table are a CRRA value-ratio welfare index. Because the model includes non-homothetic bequests and a consumption floor, these values should be read as directional access-value statistics rather than exact compensating variation; the ranking and signs across scenarios are preserved by the approximation.
...
\begin{tablenotes}
\small
\item Production model at the no-behavioral baseline (rational + preferences + $\chi_{\text{LTC}} = \pChiLTC$); the bundled behavioral wedge is applied as multiplicative post-processing ($\rho = \pWedgeFactorMid$ at the production midpoint).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\paragraph{Grid convergence.} At the \pNQuad-node production quadrature rule, predicted ownership ranges from 25.6\% at the medium grid ($60 \times 20$) to 23.6\% at the fine grid ($100 \times 40$), bracketing the production grid's no-behavioral baseline (Table~\ref{A-tab:full_robustness_continued}). The coarse grid ($40 \times 15$) deviates more substantially (29.6\%) and is reported only as a diagnostic. The 2 pp variation across the production-and-finer range is small relative to the UK retention sensitivity range that drives the bundled-wedge bracket.
...
Results have a three-layer structure. Six standard rational channels predict \ownSixChannel{} ownership relative to a frictionless benchmark of \ownFrictionless{}. Two preference channels---age-varying consumption needs and state-dependent utility---bring this to \ownEightChannelExt{}. Adding the structural public-care aversion channel ($\chi_{\text{LTC}}$) yields the no-behavioral baseline of \ownNoBehavioralBaseline{}. The bundled behavioral wedge is then applied to this baseline as multiplicative post-processing under proportional-wedge transport: predicted US ownership equals the no-behavioral baseline times the UK retention factor $\rho = \pWedgeFactorMid$ at the production midpoint, yielding \ownHeadline. The bundle is not parameterized inside the model; the wedge is implemented as a deterministic scalar reduction. The decomposition into named components (SDU, PED, choice-architecture salience) is conceptual; a single bundling moment cannot separately attribute the swing to any one component. Each layer is a substantive contribution, and the calibration is disciplined out of sample at every stage: no US moment is used to set the bundle.

Exact Shapley values, computed over all $2^9 = 512$ subsets of the nine model-internal channels, provide the paper's preferred attribution for the no-behavioral baseline. Among demand-suppressing channels, pricing loads and the combined Med+R-S health-mortality channel are the two largest contributors; age-varying needs, survival pessimism, and inflation erosion form the next tier. Social Security enters with a negative Shapley value, raising annuity demand in frictionless environments but crowding out private annuitization once market frictions are active. The bundled behavioral wedge does not appear in the Shapley enumeration: it is applied as multiplicative post-processing on ownership rates, separate from the channel-by-channel attribution.

Welfare and policy implications follow from the same calibration. Under current pricing, the welfare gain from annuity market access alone is small (under DFJ bequests, \cevGoodTwoHundredKDFJ{} at \$200{,}000 in Good health), because most calibrated households would not optimally buy at current prices. Larger welfare gains require policy reforms that change the optimal choice: group pricing roughly quadruples CEV at moderate wealth, and the best-feasible welfare package (group pricing combined with default architecture, which removes the bundled behavioral wedge) reaches \cevGoodTwoHundredKBestFeas{} at \$200{,}000. The welfare interpretation of removing the bundled wedge is normatively contingent on treating the underlying SDU/PED/salience phenomena as preference rather than bias. Two distinct policy levers operate on different margins: supply-side reforms (group pricing) raise the value of annuitization; demand-side reforms (default architecture, removing the bundled wedge) operate on the wedge magnitude.
