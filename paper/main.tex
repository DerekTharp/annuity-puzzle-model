...
Both HRS measures are reported as out-of-sample comparators for the nine-channel structural baseline (\ownNineChannelLTC). Section~\ref{sec:results} reports headline results against both measures.
...
Under the baseline calibration ($\gamma = \pGamma$, MWR $= \pMwrBaseline$), the six standard rational channels predict \ownSixChannel{} ownership relative to a frictionless population benchmark of \ownFrictionless{}---above both empirical targets and echoing the residual overprediction in \citet{pashchenko2013}. The two preference channels bring the prediction to \ownEightChannelExt{}, and adding the structural public-care aversion channel yields the nine-channel structural baseline of \ownNineChannelLTC{}, modestly above the two HRS empirical comparators ($\pctHRSIannPooled$ on the conventional any-annuity income proxy, 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$; $\pctHRSLifetime$ on the cleaner lifetime-contract indicator, 95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$). The disciplined reading is the nine-channel structural baseline, not a moment-matched fit.
