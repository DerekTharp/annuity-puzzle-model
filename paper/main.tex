...
The structural ancestor of Force B is \citet{huscott2007}, who showed analytically that cumulative prospect theory loss aversion---applied to the annuity reservation price---substantially reduces optimal annuitization in a stylized two-period setting. The present paper operationalizes the Hu--Scott mechanism in a calibrated multi-period lifecycle environment and identifies its parameter from a natural experiment (the UK 2015 reform) rather than from generic CPT priors. The contribution is empirical operationalization, not the original behavioral channel.

\paragraph{Calibration of \texorpdfstring{$\psi_{\text{purchase}}$}{psi}.} We calibrate $\psi_{\text{purchase}}$ via single-moment SMM to behavioral evidence from the UK 2015 pension freedoms reform---an external natural experiment that flipped the choice architecture from compulsory annuitization to opt-in. UK annuity sales fell roughly 75--87 percentage points \citep{abi2020}, of which approximately 15--25 pp reflects a rational response to the simultaneous removal of a 55\% tax penalty on lump-sum withdrawals; the remaining 55--65 pp is the behavioral residual attributable to architecture change alone. Targeting the midpoint of this behavioral range (60 pp) implies $\psi_{\text{purchase}} = \pPsiBracketLow$ (the conservative ABI-anchored bracket end). A more aggressive ELSA microdata-anchored sensitivity bound, which adopts the full 88 pp pre/post drop without rational stripping, implies $\psi_{\text{purchase}} = \pPsiBracketHigh$ (Appendix~\ref{A-app:psi_anchors}). The headline calibration bracket $\psi_{\text{purchase}} \in [\pPsiBracketLow, \pPsiBracketHigh]$ corresponds to a predicted US ownership bracket of $[\ownBracketLow, \ownBracketHigh]$. The wider sensitivity interval across alternative single-anchor SMM specifications spans $[0.014, 0.040]$.
...
This paper built a lifecycle model that begins from a standard rational and preference-based account of annuity demand and then \emph{adds two behavioral channels} grounded in independent empirical evidence. Without the behavioral additions, the model substantially overshoots both empirical targets; with them, the model can rationalize observed ownership under externally anchored behavioral specifications. Predicted US voluntary ownership lies in the bracket $[\ownBracketLow, \ownBracketHigh]$, anchored entirely to UK microdata. Both standard HRS measures of US lifetime annuity ownership are consistent with the bracket: $\pctHRSIannPooled$ (95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$, the conventional any-annuity income proxy) sits comfortably inside it; $\pctHRSLifetime$ (95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$, the cleaner lifetime contract indicator) overlaps the bracket's lower edge.
