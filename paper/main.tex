...
The full eight-channel rational+preference model therefore predicts \ownEightChannelExt{} ownership, a reduction from the \ownFrictionless{} frictionless benchmark but still above both empirical targets ($\pctHRSIannPooled$ on the conventional any-annuity income proxy; $\pctHRSLifetime$ on the cleaner lifetime contract indicator). Adding the structural public-care aversion channel ($\chi_{\text{LTC}} = \pChiLTC$) yields the nine-channel structural baseline of \ownNineChannelLTC{} at the baseline calibration (its sensitivity to pricing and risk aversion is taken up in Section~\ref{sec:behavioral_results}). The remainder of the paper uses the FLN central mapping as the production calibration.
