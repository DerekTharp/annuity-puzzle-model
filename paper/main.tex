...
\begin{abstract}
\noindent
\citet{yaari1965} showed that a consumer with uncertain lifetime and no bequest motive should fully annuitize under actuarially fair pricing. Observed voluntary annuity ownership among US retirees is only 3--6\%. I build a lifecycle model nesting eleven channels proposed in the annuity-puzzle literature and quantify their joint contribution using exact Shapley values computed over all 2{,}048 channel subsets. The model layers in three blocks: seven rational channels (Social Security, bequest motives, medical expenditure risk, health-mortality correlation, survival pessimism, pricing loads, inflation erosion), two preference-based channels (age-varying consumption needs, state-dependent utility), and two behavioral channels that operate at distinct decision moments and point in opposite directions: source-dependent utility (Force A; \citealp{blanchett2024,blanchett2025}) raises annuitization by converting portfolio wealth into ``spendable income,'' while a narrow-framing purchase penalty (Force B; \citealp{barberishuang2009,tverskykahneman1992}) suppresses it through loss aversion over the unrecouped premium until breakeven. Under modern annuity pricing (MWR $= \pMwrBaseline$, \citealp{wettstein2021}), the seven rational channels reduce predicted ownership from a frictionless population benchmark of \ownFrictionless{} to \ownSevenChannel{}; the preference channels bring the prediction to \ownNineChannel{}; activating Force A alone raises it to \ownTenChannel; activating Force B yields the full eleven-channel production prediction of \ownElevenChannel. Both behavioral forces are individually large in absolute magnitude (Force A roughly +55 pp, Force B roughly $-60$ pp) but partly offset, and they enter the exact Shapley decomposition with opposite signs (\shapSDU{} vs.\ \shapNarrowFraming{} pp), confirming that they are empirically distinct mechanisms rather than redundant parameterizations of the same wedge. The narrow-framing penalty $\psi_{\text{purchase}}$ is calibrated by single-moment SMM to the rational-corrected behavioral elasticity from the UK 2015 pension-freedoms reform (60 pp drop, midpoint of the 55--65 pp range after stripping the rational tax-effect component); alternative single-anchor SMM specifications yield $\psi_{\text{purchase}} \in [0.014, 0.028]$ and a corresponding range of predicted US voluntary ownership of $[2.2\%, 22.2\%]$, bracketing the observed HRS rate of \pctHRSObserved{} from above and below (Appendix~\ref{A-app:psi_anchors}). The model's implied default-vs-opt-in elasticity also matches the Goda-Manchester-Sojourner federal default-vs-opt-in evidence ($\approx$86 pp gap), which was not used in calibration. Policy counterfactuals show that supply-side reform (group pricing) and demand-side reform (default annuitization) operate on distinct margins and are quantitatively comparable as policy levers.
...
The results have a three-layer structure. First, the seven standard rational channels reduce predicted ownership from a frictionless participation benchmark of \ownFrictionless{} to \ownSevenChannel{} under modern annuity pricing (MWR $= \pMwrBaseline$). That is a substantial reduction, but it still overpredicts observed ownership by a wide margin, echoing the residual gap in \citet{pashchenko2013}. Second, adding age-varying consumption needs and state-dependent utility---the two preference channels---lowers predicted ownership to \ownNineChannel{}. Third, adding the two behavioral channels produces a striking countervailing pattern: source-dependent utility raises predicted ownership to \ownTenChannel{} by unlocking the ``license to spend'' on annuity income, while the narrow-framing purchase penalty subtracts even more demand, leaving the full eleven-channel prediction at \ownElevenChannel{}. Each behavioral force is individually large in absolute magnitude (Force A roughly $+55$ pp, Force B roughly $-60$ pp at the production calibration) but they partly offset; the net behavioral effect on the headline (about $-5$ pp in the sequential decomposition) is smaller than either force in isolation. None of the three layers depends on tuning a free parameter to match observed ownership: $\lambda_W$ is set to the cross-paper Blanchett--Finke spending differential, and $\psi_{\text{purchase}}$ is calibrated by single-moment SMM to the rational-corrected behavioral elasticity from the UK 2015 pension freedoms reform. Across alternative single-anchor SMM specifications, $\psi_{\text{purchase}} \in [0.014, 0.028]$ and the predicted US ownership ranges from $2.2\%$ to $22.2\%$, bracketing the observed HRS rate of \pctHRSObserved{} from above and below.
...
The policy and welfare results extend beyond the ownership match. Group annuity pricing (MWR $= 0.90$) raises predicted ownership from \ownElevenChannel{} to \ownGroupPricing{}, while removing the narrow-framing penalty entirely (default architecture; $\psi_{\text{purchase}} = 0$) raises it to \ownTenChannel{}---a default-vs-opt-in elasticity in line with the UK 2015 reform and the Goda-Manchester-Sojourner federal Thrift Savings Plan natural experiment, neither used in calibration. Social Security cut counterfactuals reveal that a 23\% benefit cut---the projected consequence of trust fund exhaustion around 2033---raises private annuity demand to \ownSSCutTwentyThree{}, but the largest cuts eventually reduce demand by removing the income floor that makes private annuitization feasible at all. Welfare gains from market access are concentrated at moderate wealth: the consumption-equivalent variation peaks around \cevGoodTwoHundredKDFJ{} at \$200{,}000 under the DFJ bequest specification and falls toward zero at very high wealth as the rational marginal household already takes a positive annuitization fraction.
...
\paragraph{Out-of-sample test.} The calibrated $\psi_{\text{purchase}}$ uses no US ownership information. The resulting US predicted voluntary annuitization rate of \ownElevenChannel{} is therefore an out-of-sample test against the observed HRS rate of \pctHRSObserved{}. The model overshoots observed ownership by roughly 14 pp; this residual is informative about US-specific factors not in the UK calibration (advisor incentives, distribution channel frictions, and product complexity that the structural model does not separately represent). Across alternative single-anchor SMM specifications, the predicted ownership ranges from $2.2\%$ to $22.2\%$ (Appendix~\ref{A-app:psi_anchors}), bracketing the observed \pctHRSObserved{} from above and below.
...
The welfare gains are highly heterogeneous in wealth and bequest motive. For households without bequest motives in Good health, CEV peaks at moderate wealth: \cevGoodTwoHundredKNoBq{} at \$200{,}000, falling toward zero at very high wealth as the rational marginal household already chooses positive annuitization. With the DFJ bequest specification, the pattern is similar but shifted slightly: \cevGoodTwoHundredKDFJ{} at \$200{,}000 and \cevGoodFiveHundredKBaseline{} at \$500{,}000. Even under strong bequest motives, CEV reaches \cevGoodTwoHundredKStrong{} at \$200{,}000.

At the population level, mean CEV under DFJ bequests is \popMeanCevDFJ, with \popFracPosDFJ{} of households deriving any positive welfare gain and \popFracAboveOneDFJ{} deriving gains exceeding 1\% of consumption. Without bequests, mean CEV rises to \popMeanCevNoBq{} and \popFracPosNoBq{} of households benefit.

At the same time, the table shows why aggregate ownership is low. CEV is zero for all household types with wealth at or below \$100{,}000, regardless of bequest specification. For these households, pricing loads, inflation erosion, pre-existing Social Security coverage, and the narrow-framing penalty leave little incremental value from the private nominal annuity. The zero-or-near-zero CEV at the top of the wealth distribution reflects a different mechanism: the rational marginal household at \$1{,}000{,}000 already chooses a positive optimal $\alpha$ even at the production calibration, so the welfare value from \emph{access} to the annuity market is small at the upper tail despite the positive optimal allocation.
...
\paragraph{Welfare under reform.} Under baseline pricing, meaningful CEV appears at moderate wealth (\cevGoodTwoHundredKBaseline{} for Good health with DFJ bequests at \$200{,}000). Group pricing roughly doubles the welfare gain at moderate wealth: \cevGoodTwoHundredKGroupPrice{} at \$200{,}000 (Good health, DFJ). The best feasible package extends gains across the lower wealth distribution: \cevGoodHundredKBestFeas{} at \$100{,}000 and \cevGoodTwoHundredKBestFeas{} at \$200{,}000. CEV falls to roughly zero at \$1{,}000{,}000 because the marginal household at high wealth already chooses positive optimal $\alpha$ at the production calibration, so additional access yields only small Bellman-derivative welfare gains; the result is consistent with the well-known finding that wealthy households' welfare gain from \emph{access} to a market is small once they can substitute around the constraint.

The welfare analysis reframes the annuity puzzle. For the median retiree under current market conditions and the production calibration, non-annuitization is not a mistake; it is the correct response to loaded pricing, nominal payment erosion, correlated health and survival risk, pre-existing Social Security coverage, declining consumption needs, and the at-purchase narrow-framing penalty. The welfare case for annuitization is concentrated at moderate wealth ($\$100$K--$\$500$K) and becomes substantial under supply-side reform combined with default architecture.
...
The result has a three-layer structure. Seven standard rational channels reduce predicted ownership to \ownSevenChannel, accounting for most of the gap from the frictionless benchmark. Two preference channels---age-varying consumption needs and state-dependent utility---reduce it further to \ownNineChannel. The two behavioral channels then operate in opposite directions: source-dependent utility (calibrated externally from \citet{blanchett2025}'s spending-differential evidence) raises predicted ownership to \ownTenChannel by unlocking the post-purchase ``license to spend,'' while the narrow-framing purchase penalty (calibrated by single-moment SMM to the rational-corrected behavioral elasticity from the UK 2015 pension freedoms reform) yields the full eleven-channel prediction of \ownElevenChannel. Each layer is a substantive contribution, and the calibration is identified out of sample at every stage: no parameter is tuned to match observed US ownership. Across alternative single-anchor SMM specifications for $\psi_{\text{purchase}} \in [0.014, 0.028]$, predicted US ownership ranges from $2.2\%$ to $22.2\%$, bracketing the observed \pctHRSObserved{} from above and below.
...
The welfare and policy implications are heterogeneous. Under current pricing, the welfare gain from annuity market access peaks at moderate wealth (\cevGoodTwoHundredKDFJ{} at \$200{,}000 under DFJ bequests) and falls toward zero at very high wealth where the rational marginal household already chooses positive optimal annuitization. Two distinct policy levers operate on different margins: supply-side reforms (group pricing) raise the value of annuitization, and demand-side reforms (default architecture) remove its psychological cost. Both move predicted ownership substantially.
