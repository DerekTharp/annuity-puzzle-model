...
Inflation has competing effects in the nominal-annuity calibration: it erodes the real value of later payments, but it also raises the initial nominal payout through the insurer's pricing, which uses the nominal discount rate. The net effect on demand depends on which side dominates at the calibrated parameters. At the baseline $\gamma = \pGamma$, predicted ownership rises to \ownInflationOne{} at $\pi = 1\%$ and falls to \ownInflationThree{} at $\pi = 3\%$, suggesting that erosion dominates above roughly the baseline rate but pricing dominates below it.
