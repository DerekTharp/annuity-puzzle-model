...
The results have a three-layer structure. First, the seven standard rational channels reduce predicted ownership from a frictionless participation benchmark of \ownFrictionless{} to \ownSevenChannel{} under modern annuity pricing (MWR $= \pMwrBaseline$). That is a substantial reduction, but it still overpredicts observed ownership by a wide margin, echoing the residual gap in \citet{pashchenko2013}. Second, adding age-varying consumption needs and state-dependent utility---the two preference channels---lowers predicted ownership to \ownNineChannel{}. Third, adding the two behavioral channels produces a striking countervailing pattern: source-dependent utility raises predicted ownership to \ownTenChannel{} by unlocking the ``license to spend'' on annuity income, while the narrow-framing purchase penalty subtracts even more demand, leaving the full eleven-channel prediction at \ownElevenChannel{}. Each behavioral force is large in isolation but they net to a moderate effect comparable in magnitude to the rational stack. None of the three layers depends on tuning a free parameter to match observed ownership; both behavioral parameters are anchored to independent empirical moments.
...
The exact Shapley decomposition is the paper's preferred attribution result. Pricing loads have the largest Shapley value (\shapLoads{} pp, \shapShareLoads{} of the full ownership reduction), followed by the Reichling--Smetters health-mortality correlation (\shapRS{} pp, \shapShareRS), survival pessimism (\shapPessimism{} pp, \shapSharePessimism), age-varying consumption needs (\shapAgeNeeds{} pp, \shapShareAgeNeeds), inflation erosion (\shapInflation{} pp, \shapShareInflation), and bequest motives (\shapBequests{} pp, \shapShareBequests). Social Security has a negative Shapley value ($\shapSS$ pp), reflecting its role as a complement to annuitization in many channel subsets. State-dependent utility contributes negligibly (\shapStateUtil{} pp), so it enters the paper mainly as a completeness check rather than a main driver.

The policy and welfare results extend beyond the ownership match. Group annuity pricing (MWR $= 0.90$) raises predicted ownership from \ownSevenChannel{} to \ownGroupPricing{}. A 23\% Social Security benefit cut---the projected consequence of trust fund exhaustion around 2033---raises private annuity demand to \ownSSCutTwentyThree{}, but larger cuts eventually reduce demand by removing the income floor that makes private annuitization feasible. The welfare gains from market access are concentrated among wealthier households: the consumption-equivalent variation reaches \cevGoodOneMillNoBq{} at \$1,000,000 without bequest motives and \cevGoodOneMillDFJ{} under the DFJ specification.
...
Purchase-event disutility & $\psi_{\text{purchase}}$ & \pPsiPurchase & \citet{blanchett2025}; \citet{chalmersreuter2012} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
\paragraph{Behavioral purchase friction.} The production calibration sets $\psi_{\text{purchase}} = \pPsiPurchase$, anchored to the spending-differential evidence in \citet{blanchett2025}. The empirical literature supports a wider range: the \citet{chalmersreuter2012} default-vs-opt-in gap is fully captured at $\psi_{\text{purchase}} \approx \pPsiChalmersReuterFull$, while half the gap corresponds to $\psi_{\text{purchase}} \approx \pPsiChalmersReuterMid$. The choice of $\pPsiPurchase$ is conservative within this range. Section~\ref{sec:robustness} reports the full sensitivity. A key feature of this calibration approach: $\psi_{\text{purchase}}$ is identified from a moment (spending differential or default-vs-opt-in gap) that is distinct from the prediction target (voluntary ownership level). The calibration is not tuned to match observed ownership.
...
The full nine-channel model therefore predicts \ownNineChannelFLN{} ownership under the raw FLN central mapping and \ownNineChannelRS{} under the Reichling--Smetters translation, both close to the 3--6\% range reported by \citet{lockwood2012} and \citet{dushiwebb2004} and a substantial improvement over the \pctHRSObserved{} observed in the present sample. The remainder of the paper uses the raw FLN mapping as the production calibration.

\subsection{Behavioral friction extension}
\label{sec:behavioral_results}

The nine-channel rational model overpredicts observed ownership by roughly a factor of two. The accumulated behavioral evidence---summarized in Section~\ref{sec:behavioral}---attributes the residual gap to a purchase-event friction that pricing alone cannot explain. The model captures this through the parameter $\psi_{\text{purchase}}$, calibrated externally from the spending-differential evidence in \citet{blanchett2025}.

Activating the source-dependent utility channel ($\lambda_W = \pLambdaW$) raises predicted ownership from \ownNineChannelFLN{} to \ownTenChannel{} by unlocking the post-purchase ``license to spend'' on annuity income. Activating the narrow-framing purchase penalty ($\psi_{\text{purchase}} = \pPsiPurchase$) on top of source-dependent utility yields the full eleven-channel prediction of \ownElevenChannel{}. The full eleven-channel model is therefore the paper's production specification. Crucially, both behavioral parameters are identified out of sample: $\lambda_W$ from the Blanchett--Finke spending differential, $\psi_{\text{purchase}}$ from the Tversky--Kahneman loss-aversion magnitude composed with narrow-framing structure---neither is set from observed annuitization rates.

Section~\ref{sec:robustness} reports the full sensitivity to $\psi_{\text{purchase}}$ across the empirically supported range. At the lower bound ($\psi_{\text{purchase}} = \pPsiLight$), the model predicts \ownTenChannelPsiLight; at the Chalmers--Reuter calibration ($\psi_{\text{purchase}} = \pPsiChalmersReuterMid$), it predicts \ownTenChannelPsiChalmersReuterMid. The observed rate \pctHRSObserved{} is bracketed by the two calibrations from independent literatures, supporting the dissolution claim without requiring tight identification of the behavioral parameter.
...
Table~\ref{tab:shapley_exact} reports the exact order-independent attribution results. Pricing loads have the largest Shapley value (\shapLoads{} pp, \shapShareLoads{} of the total ownership reduction), confirming their dominance regardless of decomposition order. The Reichling--Smetters health-mortality correlation is second (\shapRS{} pp, \shapShareRS), followed by survival pessimism (\shapPessimism{} pp, \shapSharePessimism), age-varying consumption needs (\shapAgeNeeds{} pp, \shapShareAgeNeeds), inflation erosion (\shapInflation{} pp, \shapShareInflation), and bequest motives (\shapBequests{} pp, \shapShareBequests). Medical expenditure risk contributes \shapMedical{} pp (\shapShareMedical), and state-dependent utility is negligible (\shapStateUtil{} pp).

Social Security has a Shapley value of $\shapSS$ pp, reflecting its role as a complement in many channel subsets: it raises demand by providing an income floor that enables annuitization. The shares sum to more than 100\% because channels interact super-additively.
...
The single-parameter sweeps above hold all other parameters at their calibrated values. To assess robustness to joint calibration uncertainty, I draw \nMCDraws{} parameter vectors from independent uniform distributions on the empirically supported ranges: $\mu_P \sim U(2.0, 3.5)$, $\pi \sim U(0.015, 0.025)$, MWR $\sim U(0.83, 0.91)$, $\psi \sim U(0.97, 1.0)$, $\delta_c \sim U(0.01, 0.03)$, and $\psi_{\text{purchase}} \sim U(0.30, 1.00)$. Risk aversion is held fixed at the baseline. For each draw the full ten-channel model is solved on the production grid and predicted ownership recorded.

\input{../tables/tex/monte_carlo_summary.tex}

The headline eleven-channel prediction of \ownElevenChannel{} sits within a 90\% confidence interval of [\mcLowCIOwnership, \mcHighCIOwnership] (median \mcMedianOwnership, IQR [\mcLowIQROwnership, \mcHighIQROwnership]) under joint parameter uncertainty. The MC median lies above the production point estimate because the priors place uniform weight across $\psi_{\text{purchase}} \in [0.30, 1.00]$, while the production calibration uses the lower end ($\psi_{\text{purchase}} = \pPsiPurchase$, anchored to Tversky--Kahneman loss aversion combined with the Blanchett--Finke survey discount). The HRS observed rate of \pctHRSObserved{} sits within the empirical bracket: the rational benchmark (\ownNineChannel) brackets observed ownership from above, the full eleven-channel model (\ownElevenChannel) brackets from below, and the SDU-only intermediate (\ownTenChannel) shows the magnitude of Force A's contribution. This robustness check shows that the predicted-versus-observed gap is sensitive primarily to the behavioral parameters, not to the rational and preference channels.
...
The welfare gains are highly heterogeneous. For households without bequest motives, CEV reaches \cevGoodTwoHundredKNoBq{} at \$200,000 and \cevGoodOneMillNoBq{} at \$1,000,000 in Good health. With the DFJ bequest specification, CEV remains substantial at the top of the wealth distribution, reaching \cevGoodOneMillDFJ{} at \$1,000,000 in Good health. Even with strong bequest motives, CEV reaches \cevGoodOneMillStrong{} at \$1,000,000. The positive welfare case for annuitization is therefore real, but concentrated among wealthier households.
...
\paragraph{Pricing loads as the dominant lever.} Group annuity pricing (MWR $= 0.90$), achievable through employer plans or government-sponsored pools \citep{james2006}, raises predicted ownership from \ownSevenChannel{} to \ownGroupPricing. A public option at MWR $= 0.95$ reaches \ownPublicOption. These results quantify the pricing channel's dominance: a modest improvement in the money's worth ratio---from \pMwrBaseline{} to 0.90---raises participation by roughly the same amount as the entire age-varying-needs channel removes from the rational decomposition.

\paragraph{Inflation protection.} A real annuity at the same MWR (\pMwrBaseline) raises ownership to \ownRealNomEquiv. A TIPS-backed real annuity at MWR $= 0.78$ yields \ownRealTIPS, illustrating that even with a pricing penalty, inflation protection substantially raises demand.

\paragraph{Social Security and the complement-substitute duality.} A 23\% across-the-board SS benefit cut---the projected consequence of trust fund exhaustion around 2033---raises private annuity demand from \ownSevenChannel{} to \ownSSCutTwentyThree{} ($+17.8$ pp). Table~\ref{tab:ss_cut} reports the full schedule of SS cuts. This is a stylized across-the-board reduction; actual benefit cuts would vary by claiming history and cohort. The relationship is non-monotonic: moderate cuts reduce crowd-out and increase private annuitization, but full elimination eventually lowers ownership because it also removes the income floor that makes annuitization feasible for low-wealth households.
...
\paragraph{Welfare under reform.} Under baseline pricing, meaningful CEV appears at \$200,000+ (\cevGoodTwoHundredKBaseline{} for Good health with DFJ bequests). Group pricing raises CEV substantially: \cevGoodTwoHundredKGroupPrice{} at \$200,000 and \cevGoodOneMillGroupPrice{} at \$1,000,000 (Good health, DFJ). The best feasible package produces large gains across the wealth distribution: \cevGoodHundredKBestFeas{} at \$100,000, rising to \cevGoodOneMillBestFeas{} at \$1,000,000.
...
\paragraph{Shapley robustness.} The exact Shapley decomposition computes over all $2^{11} = 2{,}048$ channel subsets, providing an order-independent attribution. The Shapley value rankings are consistent with the sequential decomposition: pricing loads dominate regardless of methodology. The two behavioral channels enter with opposite-sign Shapley values---source-dependent utility contributes positively (raising demand) while the narrow-framing penalty contributes negatively (suppressing demand)---confirming that the two forces have distinct empirical signatures rather than redundantly capturing the same mechanism.
...
The result has a three-layer structure. Seven standard rational channels reduce predicted ownership to \ownSevenChannel, accounting for most of the gap from the frictionless benchmark. Two preference channels---age-varying consumption needs and state-dependent utility---reduce it further to \ownNineChannel. The two behavioral channels then operate in opposite directions: source-dependent utility (calibrated externally from \citet{blanchett2025}'s spending-differential evidence) raises predicted ownership to \ownTenChannel by unlocking the post-purchase ``license to spend,'' while the narrow-framing purchase penalty (calibrated externally from Tversky--Kahneman loss aversion combined with Barberis--Huang narrow framing) yields the full eleven-channel prediction of \ownElevenChannel. Each layer is a substantive contribution, and the calibration is identified out of sample at every stage: no parameter is tuned to match observed ownership.

The exact Shapley decomposition, computed over all $2{,}048$ channel subsets, provides the paper's preferred attribution. Pricing loads are the largest single demand suppressor, followed by the Reichling--Smetters health-mortality correlation. Age-varying needs, survival pessimism, inflation erosion, and the narrow-framing purchase penalty form the next tier of quantitatively important demand-suppressing channels. Source-dependent utility enters with a positive Shapley value (raising demand) reflecting the post-purchase ``license to spend'' on annuity income. Social Security plays a dual role, raising annuity demand in frictionless environments but crowding out private annuitization once market frictions are active.

The welfare and policy implications are heterogeneous. Under current pricing, annuity market access has little value for many low-wealth households but substantial value for wealthy households, with CEV reaching \cevGoodOneMillNoBq{} at \$1,000,000 without bequest motives. Two distinct policy levers operate on different margins: supply-side reforms (group pricing) raise the value of annuitization, and demand-side reforms (default architecture) remove its psychological cost. Both move predicted ownership substantially.
