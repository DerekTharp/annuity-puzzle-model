...
\noindent
$\psi_{\text{purchase}}$ is anchored to UK post-reform evidence from the 2015 pension freedoms, combining ABI aggregate sales-volume data with an ELSA microdata sensitivity bound. The 2015 reform provides unusually informative market-design variation in annuity demand, but simultaneous tax, advice, and product-market changes mean the UK estimate is interpreted as an external calibration anchor rather than a clean causal estimate of the behavioral primitive. The empirically defensible UK calibration anchor range spans the conservative ABI aggregate (a roughly 75\% proportional decline in sales 2014--2016, with about a quarter of the model-implied ownership-rate change attributable to the simultaneous removal of a 55\% lump-sum tax penalty already represented in the model's rational pricing channels) and the descriptive ELSA microdata total drop in observed disposition. Mapped through the model, this yields a bracket of predicted US voluntary ownership of $[\ownBracketLow, \ownBracketHigh]$. Two HRS measures of US lifetime annuity ownership are reported as out-of-sample comparisons: the conventional any-annuity income proxy ($\pctHRSIannPooled$) lies inside the bracket; the cleaner fat-file lifetime-contract indicator ($\pctHRSLifetime$) overlaps only its lower edge. No US moment is used to discipline $\psi_{\text{purchase}}$; the comparison with US ownership is out of sample, not a calibration target.
...
\emph{ABI aggregate (lower bound, $\psi_{\text{purchase}} = \pPsiBracketLow$).} ABI industry sales-volume data \citep{abi2020} record a roughly 75\% proportional decline in annuity sales 2014--2016. The bracket calibration treats this proportional decline as a sensitivity target, mapping it through the model into an ownership-rate change. About a quarter of the model-implied change is plausibly attributable to the simultaneous removal of a 55\% tax penalty on lump-sum withdrawals (a budget-constraint channel already represented in the model's rational pricing); the residual mapping pins $\psi_{\text{purchase}} = \pPsiBracketLow$ and predicted US ownership of \ownBracketHigh{}.
...
\paragraph{Demand-side architecture.} The narrow-framing channel admits a distinct policy lever that supply-side reforms cannot replicate: changing the choice architecture so that annuitization is the default rather than an opt-in decision. Setting $\psi_{\text{purchase}} = 0$ in the conservative (ABI-anchored) bracket calibration (everything else unchanged) raises predicted ownership to \ownNineChannelSDU. The UK 2015 pension freedoms reform---the calibration anchor for $\psi_{\text{purchase}}$---produced a roughly 75--87\% proportional decline in annuity sales when the default flipped from compulsory to opt-in \citep{abi2020}; the \citet{goda2014} federal Thrift Savings Plan annuity option records an $\approx$86 pp household-level default-vs-opt-in gap. The TSP evidence is qualitative support for sensitivity to choice architecture in retirement-decision settings, not direct validation of the annuity-purchase penalty magnitude calibrated here. Demand-side architecture is quantitatively comparable to the largest supply-side reforms considered here, while requiring no change to insurer pricing or product design. The two policy levers operate on distinct margins: pricing reform raises the value of annuitization; default architecture removes the psychological cost of choosing it.
