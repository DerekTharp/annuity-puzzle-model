...
\begin{abstract}
\noindent
\citet{yaari1965} showed that a consumer with uncertain lifetime and no bequest motive should fully annuitize under actuarially fair pricing. Observed voluntary annuity ownership among US retirees is only a few percent. I develop two complementary models that approach the gap from independent directions. \textbf{Model 1} is a structural lifecycle decomposition. A nine-channel rational-plus-preference specification (Social Security, bequests, combined medical expenditure risk and the Reichling--Smetters health-mortality correlation, subjective survival pessimism, pricing loads, inflation erosion, public-care aversion, age-varying consumption needs from \citealp{aguiarhurst2013}, and state-dependent utility from \citealp{finkelsteinluttmer2013}) predicts ownership of \ownNineChannelLTC{} relative to a frictionless population benchmark of \ownFrictionless. This is plausibly close to the two HRS empirical comparators reported in parallel---\pctHRSLifetime{} (95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$) for the cleaner fat-file lifetime contract indicator and \pctHRSIannPooled{} (95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$) for the conventional any-annuity income proxy used in prior literature---with a small residual relative to the conventional measure.

\noindent
Layering two behavioral channels onto the nine-channel baseline yields an eleven-channel extended specification: source-dependent utility (SDU; \citealp{blanchett2024, blanchett2025}) with $\lambda_W = \pLambdaW$, and a narrow-framing at-purchase penalty (PED) consistent with \citet{brown2008framing}, \citet{chalmersreuter2012}, and \citet{huscott2007} with $\psi_{\text{purchase}} = \pPsiPurchase$. Both behavioral parameters are exploratory best guesses anchored to literature magnitudes rather than moment-matched to US data; the eleven-channel specification predicts \ownElevenChannelFull{} because the at-purchase penalty saturates at the chosen literature magnitude. The nine-channel baseline---not the eleven-channel extended specification---is the disciplined Model 1 reading.
...
The full eight-channel rational+preference model therefore predicts \ownNineChannelFLN{} ownership under the FLN central mapping and \ownNineChannelRS{} under the Reichling--Smetters translation, a reduction from the \ownFrictionless{} frictionless benchmark but still above both empirical targets ($\pctHRSIannPooled$ on the conventional any-annuity income proxy; $\pctHRSLifetime$ on the cleaner lifetime contract indicator). Adding the structural public-care aversion channel ($\chi_{\text{LTC}} = \pChiLTC$) yields the nine-channel structural baseline of \ownNineChannelLTC, plausibly close to both HRS comparators. The remainder of the paper uses the FLN mapping as the production calibration.

\subsection{Behavioral extensions and the Model 2 transport}
\label{sec:behavioral_results}

The eight-channel rational+preference model predicts \ownEightChannelExt{} ownership. Adding the structural public-care aversion channel ($\chi_{\text{LTC}} = \pChiLTC$) yields the nine-channel structural baseline of \ownNineChannelLTC{}, plausibly close to the two HRS empirical comparators ($\pctHRSLifetime$ on the lifetime contract indicator, $\pctHRSIannPooled$ on the conventional any-annuity income proxy). The nine-channel baseline accounts for most of the gap through standard mechanisms; a small residual relative to the conventional measure remains. This is the disciplined Model 1 reading.

\paragraph{Model 1 extended: literature-magnitude behavioral channels.} Layering source-dependent utility ($\lambda_W = \pLambdaW$) and the at-purchase penalty ($\psi_{\text{purchase}} = \pPsiPurchase$) onto the nine-channel baseline yields the eleven-channel extended specification. SDU alone (added before PED) raises predicted ownership to \ownTenChannelSDU{} by transforming portfolio drawdowns into a lower-weight consumption category; introducing PED on top at literature magnitude saturates participation and produces \ownElevenChannelFull. The extended specification overshoots the empirical comparators in the opposite direction at literature parameters. Because both parameters are exploratory best guesses anchored to literature magnitudes rather than moment-matched to US targets, the eleven-channel specification is reported as illustrative rather than as the headline. Sensitivity sweeps in Section~\ref{sec:robustness} document the parameter ranges.

\paragraph{Model 2: UK reduced-form transport.} Model 2 takes the post/pre retention ratio from the UK 2015 pension-freedoms reform and applies it as a multiplicative wedge against the frictionless population benchmark of \ownFrictionless{} (Section~\ref{sec:behavioral}, equation~\ref{eq:wedge}). At the mid anchor ($\rho = \pWedgeFactorMid$), the prediction is \ownHeadline{}; across UK retention anchors $[\pUKRetentionLow, \pUKRetentionHigh]$ the bracket is $[\ownBracketLow, \ownBracketHigh]$. The Model 2 transport is post-processing on the frictionless benchmark; no behavioral parameter enters the Bellman equation.
...
Under the baseline calibration ($\gamma = \pGamma$, MWR $= \pMwrBaseline$), the six standard rational channels predict \ownSixChannel{} ownership relative to a frictionless population benchmark of \ownFrictionless{}---above both empirical targets and echoing the residual overprediction in \citet{pashchenko2013}. The two preference channels bring the prediction to \ownEightChannelExt{}, and adding the structural public-care aversion channel yields the nine-channel Model 1 baseline of \ownNineChannelLTC{}, plausibly close to the two HRS empirical comparators ($\pctHRSIannPooled$ on the conventional any-annuity income proxy, 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$; $\pctHRSLifetime$ on the cleaner lifetime-contract indicator, 95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$). Model 2's UK reduced-form transport against the frictionless benchmark of \ownFrictionless{} delivers a complementary bracket of $[\ownBracketLow, \ownBracketHigh]$ with midpoint \ownHeadline{}: the conventional HRS income proxy falls inside the bracket and the lifetime-contract indicator sits just below the lower edge. The two models reach the same neighborhood from independent directions and the disciplined reading is the parallel report of Model 1 and Model 2, not a single moment-matched figure.
...
This paper developed two complementary models of the annuity puzzle. Model 1 is a structural lifecycle decomposition. A nine-channel rational-plus-preference baseline predicts \ownNineChannelLTC{} US voluntary annuity ownership, plausibly close to the two HRS empirical comparators ($\pctHRSIannPooled$ on the conventional any-annuity income proxy, 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$; $\pctHRSLifetime$ on the cleaner lifetime-contract indicator, 95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$). An eleven-channel extended specification layers two exploratory behavioral channels---source-dependent utility ($\lambda_W = \pLambdaW$) and a narrow-framing at-purchase penalty ($\psi_{\text{purchase}} = \pPsiPurchase$)---onto the baseline at literature-magnitude parameters; the at-purchase penalty saturates participation and the eleven-channel prediction is \ownElevenChannelFull{}, overshooting the empirical comparators in the opposite direction. The nine-channel structural baseline is the disciplined Model 1 reading.
