...
\noindent
$\psi_{\text{purchase}}$ is anchored to UK post-reform evidence from the 2015 pension freedoms. The reform shifted the regulated default for DC pension pots from compulsory annuitization (pre-reform ownership rate near 95--100\%) to opt-in (post-reform voluntary retention 13--25\% across ABI/FCA estimates), producing a 70--87 percentage-point drop in OWNERSHIP RATE between the two regulatory regimes. ABI quarterly sales volumes also fell roughly 75\% over the same window, corroborating the ownership-rate evidence. The 2015 reform provides unusually informative market-design variation in annuity demand, but simultaneous tax, advice, and product-market changes mean the UK estimate is interpreted as an external calibration anchor rather than a clean causal estimate of the behavioral primitive. The empirically defensible UK calibration anchor range spans the conservative ABI aggregate (the 75 pp pre/post drop in ownership rate, with about a quarter attributable to the simultaneous removal of a 55\% lump-sum tax penalty already represented in the model's rational pricing channels) and the descriptive ELSA microdata total drop in observed disposition. Mapped through the model, this yields a bracket of predicted US voluntary ownership of $[\ownBracketLow, \ownBracketHigh]$. Two HRS measures of US lifetime annuity ownership are reported as out-of-sample comparisons: the conventional any-annuity income proxy ($\pctHRSIannPooled$) lies inside the bracket; the cleaner fat-file lifetime-contract indicator ($\pctHRSLifetime$) overlaps only its lower edge. No US moment is used to discipline $\psi_{\text{purchase}}$; the comparison with US ownership is out of sample, not a calibration target.
...
\paragraph{Calibration of \texorpdfstring{$\psi_{\text{purchase}}$}{psi}.} I discipline $\psi_{\text{purchase}}$ via single-moment SMM to UK post-reform evidence from the 2015 pension freedoms---a transported market-design calibration anchor rather than clean identification of a structural behavioral primitive. The reform shifted the regulated default for DC pension pots from compulsory annuitization (pre-reform ownership rate near 95--100\%) to opt-in (post-reform voluntary retention 13--25\%; ABI/FCA), producing a 75--87 percentage-point drop in OWNERSHIP RATE between the two regulatory regimes. ABI quarterly contract volumes also fell roughly 75\% over the same window \citep{abi2020}, providing independent corroboration of the level shift. The bracket calibration uses the pre/post pp drop in ownership rate as the identification target, applying the same magnitude pp shift to the model's no-PED baseline (rational + SDU only) to pin $\psi_{\text{purchase}}$. Approximately 15--25 percentage points of the raw UK drop is plausibly attributable to the simultaneous removal of a 55\% tax penalty on lump-sum withdrawals (a budget-constraint channel already represented in the model's rational pricing); the residual rational-corrected drop pins the conservative bracket end at $\psi_{\text{purchase}} = \pPsiBracketLow$. A more aggressive ELSA microdata sensitivity bound, which adopts the full pre/post drop in observed disposition without rational stripping, implies $\psi_{\text{purchase}} = \pPsiBracketHigh$ (Appendix~\ref{A-app:psi_anchors}). The headline calibration bracket $\psi_{\text{purchase}} \in [\pPsiBracketLow, \pPsiBracketHigh]$ corresponds to a predicted US ownership bracket of $[\ownBracketLow, \ownBracketHigh]$. The wider sensitivity interval across alternative single-anchor SMM specifications spans $[0.014, 0.040]$.
...
\emph{ABI aggregate (lower bound, $\psi_{\text{purchase}} = \pPsiBracketLow$).} The ABI/FCA evidence \citep{abi2020} documents a 75 percentage-point drop in DC-pot annuity ownership between the pre-reform compulsory regime (~95\% annuitization) and the post-reform voluntary regime (~20\% retention). ABI quarterly contract volumes fell by a similar proportional magnitude over the same window, corroborating the level shift. The bracket calibration applies the 75 pp ownership-rate drop to the model's no-PED baseline as the identification target. About a quarter of that drop (15--25 pp) is plausibly attributable to the simultaneous removal of a 55\% tax penalty on lump-sum withdrawals (a budget-constraint channel already represented in the model's rational pricing); the residual rational-corrected drop pins $\psi_{\text{purchase}} = \pPsiBracketLow$ and predicted US ownership of \ownBracketHigh{}.
