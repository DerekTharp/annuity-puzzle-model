...
\noindent
Within the bundle, $\lambda_W$ (Force A) is normalized at the value implied by the Blanchett--Finke spending differential and $\psi_{\text{purchase}}$ (Force B) is calibrated by single-moment SMM to the bundled UK target; Force C (choice-architecture salience) is implicitly absorbed into the calibrated $\psi_{\text{purchase}}$ and is therefore best read as a reduced-form parameter capturing both narrow framing and the choice-architecture sub-bundle. Counterfactuals that decompose the bundle (such as a default-architecture lever that sets $\psi_{\text{purchase}} = 0$) are reported with the explicit caveat that the decomposition reflects the production parameter normalization rather than independent data identification. Joint channel attribution uses exact Shapley values computed over all 1{,}024 channel subsets across the ten Shapley-active channels.
...
\paragraph{Source-dependent utility (Force A) within the bundle.} The Force A parameter $\lambda_W$ is one component of the jointly identified behavioral bundle (Section~\ref{sec:behavioral}). I adopt $\lambda_W = \pLambdaW$ as a production normalization, motivated by the Blanchett--Finke spending differential \citep{blanchett2024,blanchett2025}: retirees consume approximately 80\% of guaranteed income but only 50\% of common withdrawal benchmarks (e.g., the 4\% rule) from portfolio wealth, implying a utility weight on portfolio-financed consumption relative to income-financed consumption in the neighborhood of $\lambda_W \approx 0.625$ at the raw differential, and approximately $\lambda_W = 0.85$ after partialling out liquidity buffering, mental-accounting confounds, bequest preservation, and tax-timing channels already represented separately in the model. The same source-dependence parameter appears in a companion paper on Social Security claiming \citep[][under review]{tharpFPR2026}, providing a consistency check across two annuity-related decisions; that consistency does not, however, independently identify $\lambda_W$ here, because the present paper's identifying moment (the UK 2015 reform) activated the bundle as a whole rather than Force A in isolation.

The implied marginal effect of activating Force A alone on annuity demand is large: holding $\psi_{\text{purchase}} = 0$ raises predicted ownership from \ownEightChannelExt{} (eight-channel pre-behavioral baseline) to \ownNineChannelSDU{}. This is a useful diagnostic but should not be read as the channel's data-identified marginal contribution: under joint identification, the same bundled equilibrium can be produced by different parameter splits within the bundle. Section~\ref{sec:robustness} reports the sensitivity of predicted ownership to $\lambda_W$ across the empirically defensible range, holding the bundled UK target fixed by re-bisecting $\psi_{\text{purchase}}$.
...
\paragraph{Force B: narrow-framing purchase-event disutility.} Activating Force B on top of Force A under the bundled production calibration yields predicted ownership of approximately 6\% (production midpoint) within a UK retention bracket of approximately [4.6\%, 8.8\%]. The mechanism is loss aversion over the cumulative gain/loss tally on the annuity ``investment'': until cumulative payouts cross the premium (breakeven, approximately age 79), the household is underwater and experiences per-period loss-aversion disutility. The penalty's NPV at the age-65 alpha search is large enough to reverse much of the SDU boost. Under joint identification (Section~\ref{sec:behavioral}), Force B's marginal contribution to the bundled equilibrium is parameter-dependent rather than independently data-identified.
...
\paragraph{Demand-side architecture.} The behavioral bundle admits a distinct policy lever that supply-side reforms cannot replicate: changing the choice architecture so that annuitization is the default rather than an opt-in decision. Setting $\psi_{\text{purchase}} = 0$ within the bundled production calibration (everything else unchanged, including $\lambda_W = \pLambdaW$) raises predicted ownership from approximately 6\% to \ownNineChannelSDU{}. This decomposition is illustrative rather than data-identified: under joint identification, the ``remove $\psi_{\text{purchase}}$'' counterfactual reflects an assumed parameter split within the bundle, and a different normalization would produce a different decomposition while reaching the same bundled equilibrium. The counterfactual conflates Force B (narrow framing) with Force C (choice-architecture salience, which is implicitly absorbed into the calibrated $\psi_{\text{purchase}}$); both move in the same direction under a default-architecture intervention---narrow framing operates on the at-purchase loss tally, choice-architecture salience operates on the surrounding decision environment---so the joint magnitude is interpretable even though the components are not separately identified. The UK 2015 pension freedoms reform---the calibration anchor for the bundle---produced a 75--87 percentage-point drop in DC-pot annuity ownership when regulatory compulsion was removed (the pre-reform regime enforced annuitization through a 55\% tax penalty on alternative withdrawals; the post-reform regime is opt-in with marginal-rate tax neutrality) \citep{abi2020}; the \citet{goda2014} federal Thrift Savings Plan annuity option records an $\approx$86 pp household-level default-vs-opt-in gap, a quasi-experimental signal that maps primarily into Force C in this paper's taxonomy. The TSP evidence is qualitative support for sensitivity to choice architecture in retirement-decision settings, not direct validation of the annuity-purchase penalty magnitude calibrated here. Demand-side architecture is quantitatively comparable to the largest supply-side reforms considered here, while requiring no change to insurer pricing or product design. The two policy levers operate on distinct margins: pricing reform raises the value of annuitization; default architecture removes the psychological cost of choosing it.
...
The behavioral content of the model lives in a jointly identified bundle containing three named forces: source-dependent utility (Force A, $\lambda_W$), narrow-framing purchase-event disutility (Force B, $\psi_{\text{purchase}}$), and choice-architecture salience (Force C, unparameterized residual). Force A is normalized at $\lambda_W = \pLambdaW$ following the Blanchett--Finke spending differential, the same value used in a companion paper on Social Security claiming \citep[][under review]{tharpFPR2026}; cross-paper consistency at this normalization provides a useful coherence check but does not, in this paper, independently identify $\lambda_W$, because the UK 2015 reform---the present paper's identifying moment---activated all three behavioral phenomena at once when compulsion lifted. Force B is calibrated by single-moment SMM such that the model's combined behavioral effect on the eight-channel pre-behavioral baseline (\ownEightChannelExt{}) matches the bundled UK target of approximately 6\% at the production midpoint; Force C's empirical magnitude (5--10 pp in the broader literature on retirement decision architecture) is implicitly absorbed into the calibrated $\psi_{\text{purchase}}$, which is therefore best interpreted as a reduced-form parameter capturing narrow framing plus the choice-architecture sub-bundle rather than narrow framing alone. The corresponding sensitivity range across UK retention anchors maps to predicted US ownership of approximately [4.6\%, 8.8\%], with a wider range across alternative anchors of [2.5\%, 8.8\%]. Both HRS measures of US lifetime annuity ownership fall within the wider sensitivity range. The \citet{goda2014} federal Thrift Savings Plan default evidence is qualitatively consistent with the model's implied default-vs-opt-in elasticity (which combines Force B's at-purchase loss aversion with Force C's choice-architecture salience under joint identification), providing supporting context rather than a clean external validation for the bundled behavioral wedge's order of magnitude. \citet{huscott2007} showed analytically that prospect theory loss aversion reduces optimal annuitization substantially; the present paper operationalizes that mechanism (Force B) in a calibrated lifecycle environment with parameter discipline from UK post-reform microdata. \citet{ameriks2020} document the importance of long-term care aversion in late-life saving decisions, which remains to be incorporated. \citet{madrian2001} showed that defaults dominate retirement savings behavior more generally; in this paper that finding is captured by Force C within the bundle, operationalized through the demand-side counterfactual rather than as a separately identified channel.
...
Results have a three-layer structure. Six standard rational channels predict \ownSixChannel{} ownership relative to a frictionless benchmark of \ownFrictionless{}. Two preference channels---age-varying consumption needs and state-dependent utility---bring this to \ownEightChannelExt{}, still substantially above both empirical targets. The bundled behavioral wedge then closes the residual gap: under the proportional-wedge transport (UK post/pre retention ratio $\approx 0.18$ applied to the eight-channel pre-behavioral US baseline), predicted ownership is approximately 6\% at the production midpoint. Within the bundle, $\lambda_W$ (Force A) is normalized at the value implied by the Blanchett--Finke spending differential, $\psi_{\text{purchase}}$ (Force B) is calibrated by single-moment SMM to the bundled UK target, and Force C (choice-architecture salience) is implicitly absorbed into the calibrated $\psi_{\text{purchase}}$ as an unparameterized residual. The decomposition into Force A, Force B, and Force C contributions is parameter-dependent rather than data-identified, because a single moment cannot separately attribute the swing to any one channel. Each layer is a substantive contribution, and the calibration is disciplined out of sample at every stage: no US moment is used to set the behavioral bundle.
