...
% Defensive placeholders for two-model architecture macros.
% \providecommand only defines a macro if it isn't already defined in
% numbers.tex, so the manuscript compiles cleanly during the transition
% period before the pipeline regenerates numbers.tex.
\providecommand{\pChiLTC}{0.7}
...
% ---- Title ----
\title{Decomposing the Annuity Puzzle: \\
A Structural Lifecycle Model and a \\
Reduced-Form UK Transport\thanks{%
...
\begin{abstract}
\noindent
\citet{yaari1965} showed that a consumer with uncertain lifetime and no bequest motive should fully annuitize under actuarially fair pricing. Observed voluntary annuity ownership among US retirees is only a few percent. I develop two complementary models that approach the gap from independent directions. \textbf{Model 1} is a structural lifecycle decomposition. A nine-channel rational-plus-preference specification (Social Security, bequests, combined medical expenditure risk and the Reichling--Smetters health-mortality correlation, subjective survival pessimism, pricing loads, inflation erosion, public-care aversion, age-varying consumption needs from \citealp{aguiarhurst2013}, and state-dependent utility from \citealp{finkelsteinluttmer2013}) predicts ownership of \ownNineChannelLTC{} relative to a frictionless population benchmark of \ownFrictionless. This is a slight underestimate of the two HRS empirical comparators reported in parallel---\pctHRSLifetime{} (95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$) for the cleaner fat-file lifetime contract indicator and \pctHRSIannPooled{} (95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$) for the conventional any-annuity income proxy used in prior literature.

\noindent
The nine-channel result does not imply that behavioral factors do not matter. In an exploratory analysis I layer two behavioral channels onto the nine-channel baseline: source-dependent utility (SDU; \citealp{blanchett2024, blanchett2025}) with $\lambda_W = \pLambdaW$, and a narrow-framing at-purchase penalty (PED) consistent with \citet{brown2008framing}, \citet{chalmersreuter2012}, and \citet{huscott2007} with $\psi_{\text{purchase}} = \pPsiPurchase$. Both parameters are exploratory best guesses anchored to literature magnitudes rather than moment-matched to US data. At these values each behavioral channel individually moves predicted ownership by more than almost any single rational, preference, or structural channel, but the two operate in opposite directions and largely offset. The eleven-channel prediction is \ownElevenChannelFull{} and is sensitive to assumptions---small shifts within the literature-defensible range produce large changes in either direction---so the nine-channel structural baseline remains the disciplined Model 1 reading. The dynamics observed in this exercise would themselves predict a meaningful amount of inconsistency in empirical and structural estimates of annuity demand across studies and over time.

\noindent
\textbf{Model 2} is a reduced-form transport using the UK 2015 pension-freedoms reform as out-of-sample variation. The 2015 reform shifted the regulated regime for DC pension pots from de facto compulsory annuitization---enforced pre-reform by a 55\% unauthorised-payment charge on alternative withdrawals (pre-reform retention $\pUKRetentionPre$)---to voluntary access with marginal-rate tax neutrality, producing post-reform voluntary retention of $\pUKRetentionMid$ at the mid anchor (range $\pUKRetentionLow$--$\pUKRetentionHigh$ across ABI and FCA estimates). The retention ratio is applied as a behavioral wedge against the frictionless population benchmark of \ownFrictionless, yielding predicted US ownership of \ownHeadline{} (bracket $[\ownBracketLow, \ownBracketHigh]$). The empirical fact that UK annuity share remained at approximately 9\% in 2024/25 despite gilt yields at 14-year highs \citep{fca2025} indicates that the post-2015 retention regime has been broadly stable across a full rate cycle, ruling out a substantial rate-environment confound. No US moment is used to discipline Model 2; consistency with the HRS comparators is an out-of-sample comparison. The conventional HRS income proxy lies inside the Model 2 bracket; the cleaner lifetime-contract indicator sits just below it.
...
This paper provides a calibrated structural accounting exercise for that gap, supplemented by an external reduced-form transport. Model 1 is a structural lifecycle decomposition with nine rational-plus-preference channels: pre-existing Social Security annuitization, bequest motives, combined medical expenditure risk and the health-mortality correlation identified by \citet{reichlingsmetters2015}, subjective survival pessimism \citep{odeasturrock2023}, pricing loads, inflation erosion, public-care aversion \citep{ameriks2020}, age-varying consumption needs \citep{aguiarhurst2013}, and state-dependent utility \citep{finkelsteinluttmer2013}. Together these nine channels predict ownership of \ownNineChannelLTC, a slight underestimate of the two HRS empirical comparators ($\pctHRSLifetime$ and $\pctHRSIannPooled$). The structural model accounts for most of the gap through standard mechanisms, leaving a small residual relative to the conventional measure; I do not claim it ``matches'' or ``resolves'' the puzzle.

Model 1 also reports an eleven-channel extended specification that layers two behavioral channels onto the nine-channel baseline: source-dependent utility (SDU; \citealp{shefrin1988, thaler1999, blanchett2024, blanchett2025}) operationalized as a $\lambda_W = \pLambdaW$ discount on portfolio drawdowns, and a narrow-framing at-purchase penalty (PED) consistent with \citet{tverskykahneman1992}, \citet{huscott2007}, \citet{brown2008framing}, and \citet{chalmersreuter2012} parameterized as $\psi_{\text{purchase}} = \pPsiPurchase$. Both parameters are exploratory best guesses anchored to literature magnitudes; they are \emph{not} moment-matched to a US identification target. The eleven-channel specification predicts \ownElevenChannelFull{} because the at-purchase penalty saturates at the chosen literature magnitude; this overshoots the empirical comparators in the opposite direction. The nine-channel baseline is therefore the disciplined Model 1 reading; the eleven-channel specification is reported as a literature-magnitude exercise that shows where standard behavioral parameters would push predictions if introduced in isolation.

Model 2 is a reduced-form transport that bypasses internal behavioral parameterization. The UK 2015 pension-freedoms reform shifted regulated DC pension-pot decumulation from de facto compulsory annuitization (enforced by a 55\% unauthorised-payment charge on alternative withdrawals; pre-reform retention $\pUKRetentionPre$) to voluntary access with marginal-rate tax neutrality. Post-reform voluntary retention is $\pUKRetentionMid$ at the mid anchor (range $\pUKRetentionLow$--$\pUKRetentionHigh$ across ABI and FCA estimates). The post-to-pre retention ratio is applied as a multiplicative behavioral wedge against the frictionless population benchmark of \ownFrictionless, yielding predicted US ownership of \ownHeadline{} (bracket $[\ownBracketLow, \ownBracketHigh]$). FCA Retirement Income Market Data \citep{fca2025} report annuity share at approximately 9\% in 2024/25 despite gilt yields at 14-year highs, indicating that the post-2015 retention regime has been broadly stable across a full rate cycle. The English Longitudinal Study of Ageing (ELSA) pension grid provides supporting individual-level evidence: $\pctELSAWaveSixAnnuity$ of wave 6 (2012--13) DC pension recipients reported annuity-style income under the pre-freedoms mandate, and pooling waves 8--11 (2016--24), $\pctELSAPostAnnuity$ of observed lump-sum disposition records used the lump sum to buy annuity income (n=\nELSAPostLumpSum). No US moment is used to discipline Model 2; consistency with the HRS comparators is an out-of-sample comparison.

The paper's contribution is fourfold. First, the unified structural framework---the first to nest age-varying consumption needs alongside the established rational and preference channels of the annuity-puzzle literature. Second, the cross-validation of a structural decomposition against an external reduced-form transport that anchors aggregate behavioral attrition to a single market-design moment. Third, an exact Shapley decomposition over all $2^{11} = 2{,}048$ subsets of the eleven channels that attributes the demand reduction without order dependence. Fourth, two distinct policy levers emerge from the comparison: a supply-side reform (group pricing) and a demand-side reform (default architecture) operate on different margins.

The results have a three-layer structure. First, under modern annuity pricing (MWR $= \pMwrBaseline$, \citealp{wettstein2021}), the six rational channels alone predict ownership of \ownSixChannel{} relative to a frictionless population benchmark of \ownFrictionless{}; adding the two preference channels brings the prediction to \ownEightChannelExt{}; adding public-care aversion ($\chi_{\text{LTC}}$) yields the nine-channel structural baseline of \ownNineChannelLTC. Second, the eleven-channel extended specification layers SDU and PED at literature-magnitude parameters and predicts \ownElevenChannelFull{}; the at-purchase penalty saturates and the model overshoots in the opposite direction, illustrating that small-parameter behavioral additions can move predictions discontinuously and motivating the disciplined nine-channel reading. Third, Model 2 applies the UK 2015 post/pre retention ratio to the frictionless benchmark and predicts \ownHeadline{} (bracket $[\ownBracketLow, \ownBracketHigh]$). Both HRS empirical comparators ($\pctHRSIannPooled$ for the conventional income proxy; $\pctHRSLifetime$ for the cleaner lifetime-contract indicator) are reported as out-of-sample comparisons against both models.
...
The exact Shapley decomposition over the eleven channels provides the paper's preferred order-independent attribution. Among demand-suppressing channels, the at-purchase penalty (\shapPED{} pp, \shapSharePED) and pricing loads (\shapLoads{} pp, \shapShareLoads) are the two largest contributors, followed by the combined medical risk and Reichling--Smetters health-mortality correlation channel (\shapMedRS{} pp, \shapShareMedRS), age-varying consumption needs (\shapAgeNeeds{} pp, \shapShareAgeNeeds), survival pessimism (\shapPessimism{} pp, \shapSharePessimism), and inflation erosion (\shapInflation{} pp, \shapShareInflation). Source-dependent utility (\shapSDU{} pp, \shapShareSDU) and Social Security ($\shapSS$ pp, $\shapShareSS$) enter with negative Shapley values, reflecting their role as ownership boosters. State-dependent utility, bequest motives, and public-care aversion contribute negligibly. The large, opposite-sign Shapley values for SDU and PED reflect their operation on distinct decision margins; under the literature-magnitude parameterization the PED contribution dominates.

The policy and welfare results extend beyond the ownership match. Group annuity pricing (MWR $= 0.90$) raises predicted ownership relative to the production calibration. Removing the at-purchase penalty ($\psi_{\text{purchase}} = 0$) recovers a counterfactual broadly consistent with the UK 2015 reform and the \citet{goda2014} federal Thrift Savings Plan natural experiment. Social Security cut counterfactuals reveal that a 23\% benefit cut---the projected consequence of trust fund exhaustion around 2033---raises private annuity demand substantially, but the largest cuts eventually reduce demand by removing the income floor that makes private annuitization feasible at all. Under current pricing the welfare gain from annuity market access is small across the wealth distribution because most calibrated households optimally do not annuitize, so access is approximately neutral; policy interventions that change the optimal choice (group pricing, demand-side architecture) generate the larger welfare gains.
...
\subsection{Behavioral channels (exploratory) and the UK reduced-form transport}
...
\paragraph{Narrow-framing at-purchase penalty (PED; exploratory).} A cumulative-prospect-theory mechanism formalized by \citet{huscott2007}, with empirical support from \citet{brown2008framing} and \citet{chalmersreuter2012}, generates a one-time at-purchase disutility over the unrecouped premium. The model captures this as a fixed utility cost imposed at the annuitization decision:
\begin{equation}
\label{eq:ped}
\Delta V_{\text{purchase}} = -\psi_{\text{purchase}} \cdot u(c_{\text{ref}}) \cdot \mathbf{1}(\alpha > 0),
\end{equation}
...
\paragraph{Status of behavioral channels.} Both SDU and PED are exploratory rather than disciplined. The model could be moment-matched against a US annuitization target, but doing so would conflict with the out-of-sample identification strategy used elsewhere in the paper (the structural baseline uses no annuitization moment; Model 2 uses only UK data). The eleven-channel specification therefore shows what happens when these channels are introduced at literature magnitudes; the headline behavioral result is delivered by Model 2 below rather than by an eleven-channel within-model calibration.

\paragraph{Model 2: UK reduced-form transport.} Model 2 bypasses internal behavioral parameterization. The UK 2015 pension-freedoms reform shifted regulated DC pension-pot decumulation from de facto compulsory annuitization to voluntary access; pre-reform retention was $\pUKRetentionPre$ and post-reform retention is $\pUKRetentionMid$ at the mid anchor (range $\pUKRetentionLow$--$\pUKRetentionHigh$ across ABI and FCA estimates). The post-to-pre retention ratio is applied as a multiplicative wedge against the frictionless population benchmark:
\begin{equation}
\label{eq:wedge}
\text{predicted US ownership (Model 2)} = \text{frictionless US benchmark} \times \frac{\text{UK retention}_{\text{post}}}{\text{UK retention}_{\text{pre}}}.
\end{equation}
At the mid anchor, $\frac{\pUKRetentionMid}{\pUKRetentionPre} \cdot \ownFrictionless = \ownHeadline$; across UK anchors the prediction range is $[\ownBracketLow, \ownBracketHigh]$. The transport is implemented as post-processing on the frictionless benchmark in scripts/export\_manuscript\_numbers.jl; no behavioral parameter enters the Bellman equation.

\paragraph{Cross-country transport: maintained assumption.} Proportional-wedge identification is a transport assumption. The maintained assumption is that the equilibrium retention ratio (post-reform / pre-reform) is country-invariant: UK and US retirees have sufficiently similar aggregate behavioral response to the lifting of compulsory annuitization that the same proportional retention applies in both populations. This assumption is load-bearing. UK and US institutional environments differ along several dimensions not absorbed by the wedge: DC pension-pot scale and concentration, the FCA/RDR adviser regime versus US insurance-distribution structures, Medicaid/SS interaction patterns, and minimum-purchase requirements. The Model 2 bracket $[\ownBracketLow, \ownBracketHigh]$ is therefore an out-of-sample extrapolation under a transport assumption, not a clean identification result.

\paragraph{Tax-penalty and rate-environment confounds.} Two confounds within the UK identifying moment are worth flagging. First, the UK 55\% unauthorised-payment charge that enforced pre-reform compulsion is a budget-constraint mechanism not a preference parameter; the transport absorbs this into the wedge but the implied attrition therefore includes a UK-specific tax-removal response (estimated at 5--15 pp of the raw retention swing; see \citealp{cribb2018annuity, cannon2016annuity} for analyses of the UK reform's tax and behavioral components). Second, the rate-environment confound is empirically small: FCA Retirement Income Market Data \citep{fca2025} report annuity share of DC pots accessed at approximately 9\% in 2024/25 despite gilt yields at 14-year highs, indicating that the post-2015 retention regime has been broadly stable across a full rate cycle and that gilt-yield variation explains 0--5 pp at most of the post-reform retention level. The Model 2 bracket covers both confounds.

\paragraph{Out-of-sample comparison.} Model 2 is calibrated entirely from UK data, with no reference to observed US ownership. The predicted US voluntary annuitization bracket of $[\ownBracketLow, \ownBracketHigh]$ is an out-of-sample comparison. Both HRS measures of US lifetime annuity ownership ($\pctHRSLifetime$ and $\pctHRSIannPooled$) are reported alongside the prediction in Section~\ref{sec:results}.
...
The decomposition in equation~(\ref{eq:multiplicative_R}) characterizes the nine-channel structural baseline. SDU enters the value function as a separate multiplicative factor on portfolio-financed consumption (equation~\ref{eq:sdu}), and the at-purchase penalty (equation~\ref{eq:ped}) enters as a one-time additive shifter on the annuitization decision; together they contribute to the eleven-channel Shapley enumeration in Section~\ref{sec:shapley}. The Model 2 transport is applied externally to the frictionless benchmark (equation~\ref{eq:wedge}); it does not enter the Bellman equation or the Shapley enumeration.
...
\multicolumn{4}{l}{\textit{Exploratory behavioral channels (Model 1 extended)}} \\
SDU portfolio discount & $\lambda_W$ & \pLambdaW & \citet{blanchett2024,blanchett2025} (literature magnitude) \\
At-purchase penalty & $\psi_{\text{purchase}}$ & \pPsiPurchase & \citet{brown2008framing}; \citet{chalmersreuter2012} (literature magnitude) \\[4pt]
\multicolumn{4}{l}{\textit{Model 2 reduced-form transport}} \\
UK retention factor (mid) & $\rho$ & \pWedgeFactorMid & UK 2015 reform \citep{abi2020}; \citep{fca2025} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
\paragraph{Public-care aversion ($\chi_{\text{LTC}}$).} The structural channel $\chi_{\text{LTC}} = \pChiLTC$ is calibrated within the upper bound of the Ameriks et al.\ (2020 ECMA) 95\% CI for public-care utility weight. Their central estimate is 0.5; we adopt 0.7 as a conservative choice that avoids overcalibrating this channel relative to other suppression mechanisms in the model. The channel applies a utility multiplier of $\chi_{\text{LTC}} < 1$ when the consumption floor binds and the agent is in Poor health (the Medicaid-LTC binding state), capturing retirees' preference for self-financed over publicly-financed long-term care.

\paragraph{Exploratory behavioral parameters (Model 1 extended).} The two behavioral channels are calibrated to literature magnitudes rather than estimated from US moments. SDU's $\lambda_W = \pLambdaW$ follows the \citet{blanchett2024,blanchett2025} retiree spending differential (approximately 50\% from portfolio assets versus 80\% from guaranteed income, implying $\lambda_W = 50/80 = 0.625$). The at-purchase penalty $\psi_{\text{purchase}} = \pPsiPurchase$ at reference consumption $c_{\text{ref}} = \$\pPsiPurchaseCRef$ is a literature-magnitude best guess consistent with cumulative-prospect-theory parameterizations \citep{huscott2007} and the \citet{brown2008framing} and \citet{chalmersreuter2012} narrow-framing evidence. Both parameters are exploratory and not moment-matched to any US annuitization target. Sensitivity sweeps across $\lambda_W \in \{0.5, 0.625, 0.75, 0.85\}$ and $\psi_{\text{purchase}} \in \{0.01, 0.05, 0.09\}$ are reported in Section~\ref{sec:robustness}.

\paragraph{Model 2 reduced-form transport.} The UK 2015 pension-freedoms reform is the identifying moment for Model 2. The post/pre retention ratio is applied as a multiplicative wedge against the frictionless population benchmark (Section~\ref{sec:behavioral}, equation~\ref{eq:wedge}). The proportional retention factor at the mid anchor is $\rho = \pWedgeFactorMid$ (UK post-reform retention $\pUKRetentionMid$ divided by pre-reform compulsory retention $\pUKRetentionPre$). The UK retention sensitivity range of [\pUKRetentionLow, \pUKRetentionHigh] maps to a US prediction bracket of $[\ownBracketLow, \ownBracketHigh]$ when applied to the frictionless benchmark of \ownFrictionless. The English Longitudinal Study of Ageing (ELSA) pension grid provides supporting individual-level evidence: in wave 6 (2012--13, pre-freedoms), $\pctELSAWaveSixAnnuity$ of DC pension recipients reported annuity-style income under the mandate; pooling waves 8--11 (2016--24, post-freedoms), $\pctELSAPostAnnuity$ of observed lump-sum disposition records used the lump sum to buy annuity income (n=\nELSAPostLumpSum).

\emph{Out-of-sample reading.} The Model 2 calibration is set entirely from non-US data. Both HRS measures of US lifetime annuity ownership---the conventional any-annuity income proxy ($\pctHRSIannPooled$, 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$) and the cleaner fat-file lifetime-contract indicator ($\pctHRSLifetime$, 95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$)---are compared against the Model 2 bracket $[\ownBracketLow, \ownBracketHigh]$. The conventional income proxy lies inside the bracket; the lifetime contract indicator sits just below the lower edge. No US moment is used to set the calibration; consistency with US ownership is an out-of-sample comparison.
...
Both HRS measures are reported as out-of-sample comparators for Model 1 (\ownNineChannelLTC{} nine-channel structural baseline) and Model 2 (bracket $[\ownBracketLow, \ownBracketHigh]$). The conventional income proxy lies inside the Model 2 bracket; the cleaner lifetime-contract indicator sits just below it. Section~\ref{sec:results} reports headline results against both measures.
...
The full eight-channel rational+preference model therefore predicts \ownNineChannelFLN{} ownership under the FLN central mapping and \ownNineChannelRS{} under the Reichling--Smetters translation, a reduction from the \ownFrictionless{} frictionless benchmark but still above both empirical targets ($\pctHRSIannPooled$ on the conventional any-annuity income proxy; $\pctHRSLifetime$ on the cleaner lifetime contract indicator). Adding the structural public-care aversion channel ($\chi_{\text{LTC}} = \pChiLTC$) yields the nine-channel structural baseline of \ownNineChannelLTC, a slight underestimate of both HRS comparators. The remainder of the paper uses the FLN mapping as the production calibration.

\subsection{Behavioral extensions and the Model 2 transport}
\label{sec:behavioral_results}

The eight-channel rational+preference model predicts \ownEightChannelExt{} ownership. Adding the structural public-care aversion channel ($\chi_{\text{LTC}} = \pChiLTC$) yields the nine-channel structural baseline of \ownNineChannelLTC{}, a slight underestimate of the two HRS empirical comparators ($\pctHRSLifetime$ on the lifetime contract indicator, $\pctHRSIannPooled$ on the conventional any-annuity income proxy). The nine-channel baseline accounts for most of the gap through standard mechanisms, leaving a small residual relative to the conventional measure. This is the disciplined Model 1 reading.

\paragraph{Model 1 extended: literature-magnitude behavioral channels.} The nine-channel result does not imply that behavioral factors do not matter. To examine their potential role, I layer two behavioral channels onto the nine-channel baseline as an exploratory analysis: source-dependent utility ($\lambda_W = \pLambdaW$) and a narrow-framing at-purchase penalty ($\psi_{\text{purchase}} = \pPsiPurchase$). Both parameters are exploratory best guesses anchored to literature magnitudes rather than moment-matched to US targets; the headline reading remains the nine-channel structural baseline. With both channels active at these values, SDU raises predicted ownership to \ownTenChannelSDU{} by transforming portfolio drawdowns into a lower-utility-weight consumption category, and PED then saturates participation, yielding the eleven-channel prediction \ownElevenChannelFull.

Two features of this exploratory exercise are worth flagging. First, at the chosen values each behavioral channel individually moves ownership by more than almost any single rational, preference, or structural channel in the model. In absolute magnitude the Shapley contributions for PED (\shapPED{} pp) and SDU (\shapSDU{} pp, entering with a negative sign because it is a booster) exceed pricing loads (\shapLoads{} pp), the combined Med+R-S channel (\shapMedRS{} pp), and survival pessimism (\shapPessimism{} pp); see Section~\ref{sec:shapley}. Second, the two operate in opposite directions and largely offset: SDU pulls demand up, PED pulls it down, and the net effect on ownership depends sensitively on the chosen parameters. Sensitivity sweeps across $\lambda_W \in \{0.5, 0.625, 0.75, 0.85\}$ and $\psi_{\text{purchase}} \in \{0.01, 0.05, 0.09\}$ in Section~\ref{sec:robustness} show that small shifts within the literature-defensible range move the eleven-channel prediction substantially in either direction.

\paragraph{Model 2: UK reduced-form transport.} An alternative way to bound the role of behavioral factors is to look at total demand change in a setting where compulsion lifted---the UK 2015 pension-freedoms reform. Model 2 takes the post/pre retention ratio from that reform and applies it as a multiplicative wedge against the frictionless population benchmark of \ownFrictionless{} (Section~\ref{sec:behavioral}, equation~\ref{eq:wedge}). At the mid anchor ($\rho = \pWedgeFactorMid$), the prediction is \ownHeadline{}; across UK retention anchors $[\pUKRetentionLow, \pUKRetentionHigh]$ the bracket is $[\ownBracketLow, \ownBracketHigh]$. Model 2 captures whatever combination of behavioral and unmodeled rational frictions UK households actually expressed when compulsion was removed, transported to the US frictionless benchmark under the maintained assumption of country-invariant proportional retention. No behavioral parameter enters the Bellman equation; the transport is deterministic post-processing on the frictionless benchmark.

\paragraph{Out-of-sample comparison.} The conventional any-annuity income proxy ($\pctHRSIannPooled$, Wilson 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$) and the cleaner lifetime-contract indicator ($\pctHRSLifetime$, Wilson 95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$) are reported as out-of-sample comparisons against both the Model 1 nine-channel baseline (\ownNineChannelLTC) and the Model 2 bracket ($[\ownBracketLow, \ownBracketHigh]$). The conventional income proxy lies inside the Model 2 bracket; the lifetime-contract indicator sits just below it. The two models reach the same neighborhood from independent directions, providing cross-validation rather than a single moment-matched fit.

\subsection{Exact Shapley decomposition}
\label{sec:shapley}

The sequential decomposition is order-dependent: the contribution attributed to each channel depends on which channels precede it. To obtain order-independent attributions, I compute exact Shapley values by solving the model for all $2^{11} = 2{,}048$ subsets of the eleven channels and averaging each channel's marginal contribution across all possible orderings. Medical expenditure risk and the Reichling--Smetters health-mortality correlation are combined into a single channel because the R-S mechanism's quantitative bite operates through the interaction with medical risk: without competing demand for liquid wealth in sick states, the lower expected annuity NPV does not translate into a precautionary motive against annuitization.

Table~\ref{tab:shapley_exact} reports the exact order-independent attribution. Among demand-suppressing channels, the at-purchase penalty (\shapPED{} pp, \shapSharePED) and pricing loads (\shapLoads{} pp, \shapShareLoads) are the two largest contributors, followed by the combined Med+R-S health-mortality channel (\shapMedRS{} pp, \shapShareMedRS), age-varying consumption needs (\shapAgeNeeds{} pp, \shapShareAgeNeeds), survival pessimism (\shapPessimism{} pp, \shapSharePessimism), and inflation erosion (\shapInflation{} pp, \shapShareInflation). Bequest motives (\shapBequests{} pp), state-dependent utility (\shapStateUtil{} pp), and public-care aversion (\shapLTC{} pp) contribute negligibly.

Two channels enter with negative Shapley values, reflecting their role as ownership boosters: source-dependent utility (\shapSDU{} pp, \shapShareSDU) and Social Security ($\shapSS$ pp, $\shapShareSS$). Source-dependent utility raises annuitization by transforming portfolio drawdowns into a lower-utility-weight consumption category; Social Security raises annuitization by providing the income floor that makes additional annuitization rational at the margin. The shares sum to more than 100\% in absolute value because channels interact super-additively and because the negative-Shapley boosters partly offset the suppressors.

The Shapley values for SDU and PED reflect the literature-magnitude calibrations of $\lambda_W = \pLambdaW$ and $\psi_{\text{purchase}} = \pPsiPurchase$ within the eleven-channel extended specification. Because these parameters are exploratory rather than moment-matched, the Shapley attributions for SDU and PED should be read as illustrative of where literature-magnitude behavioral parameters land in the channel ranking, not as identification of the magnitudes themselves. The Model 2 transport is reported separately as an external reduced-form cross-check; it does not enter the Shapley enumeration.

\input{../tables/tex/shapley_exact.tex}

The Shapley values sharpen three findings from the sequential decomposition. First, pricing loads and the combined Med+R-S channel are the two largest model-internal demand-suppressing contributors regardless of ordering. Second, the channels not jointly incorporated in the earlier multi-channel literature---the combined Med+R-S correlation, survival pessimism, and age-varying needs---collectively account for \shapRSPessAge{} pp (\shapShareRSPessAge), comparable to pricing loads. Third, the two behavioral channels enter with large and opposite-sign Shapley values; under the exploratory literature-magnitude parameterization the PED contribution dominates and pushes the eleven-channel prediction below both empirical comparators.

\subsection{Pairwise channel interactions}

Table~\ref{tab:pairwise} reports the pairwise interaction strength for the eight rational and preference channels. For each pair (A, B), the interaction is defined as the difference between the ownership drop when both are active simultaneously and the sum of their individual drops. A negative interaction indicates super-additivity: the channels reinforce each other. The Model 2 transport is omitted from this table because it is applied as multiplicative post-processing on the frictionless benchmark rather than as a model channel; the order-independent contributions of the eleven model-internal channels are reported in the exact Shapley decomposition (Section~\ref{sec:shapley}).
...
At the baseline $\gamma = \pGamma$, the six-channel rational model predicts \ownSixChannel{}. Predicted ownership rises with $\gamma$: \ownGammaTwo{} at $\gamma = 2.0$, \ownGammaTwoPointFour{} at $\gamma = 2.4$, and \ownGammaThree{} at $\gamma = 3.0$. The eight-channel rational+preference model (with age-varying needs and state-dependent utility) predicts \ownEightChannelExt{} at the baseline $\gamma = \pGamma$. Adding the structural public-care aversion channel ($\chi_{\text{LTC}}$) yields the nine-channel structural baseline of \ownNineChannelLTC{}; applying the Model 2 transport against the frictionless benchmark yields \ownHeadline{} at the UK mid anchor with bracket $[\ownBracketLow, \ownBracketHigh]$ across UK retention anchors.
...
The single-parameter sweeps above hold all other parameters at their calibrated values. To assess robustness to joint calibration uncertainty within the model-internal parameters, I draw \nMCDraws{} parameter vectors from independent uniform distributions on the empirically supported ranges: $\mu_P \sim U(2.0, 3.5)$, $\pi \sim U(0.015, 0.025)$, MWR $\sim U(0.83, 0.91)$, $\psi \sim U(0.97, 1.0)$, $\delta_c \sim U(0.01, 0.03)$. Risk aversion is held fixed at the baseline. For each draw the model is solved on the production grid and the structural-baseline ownership prediction is recorded; the Model 2 transport is applied separately as multiplicative post-processing on the frictionless benchmark.

\input{../tables/tex/monte_carlo_summary.tex}

The structural-baseline prediction sits within a 90\% sensitivity interval of [\mcLowCIOwnership, \mcHighCIOwnership] (median \mcMedianOwnership, IQR [\mcLowIQROwnership, \mcHighIQROwnership]) under joint parameter uncertainty. The interval is reasonably tight because the joint draw is over five rational/preference parameters whose effects partly offset each other. Both HRS empirical comparators ($\pctHRSLifetime$ on the lifetime contract indicator, $\pctHRSIannPooled$ on the conventional any-annuity income proxy) are reported as out-of-sample comparisons against this range and against the Model 2 bracket of $[\ownBracketLow, \ownBracketHigh]$.
...
\paragraph{Demand-side architecture.} A demand-side lever distinct from pricing reform is changing the choice architecture so that annuitization is the default rather than an opt-in decision. The UK 2015 pension-freedoms reform produced a $87$--$89$ percentage-point drop in DC-pot annuity ownership when regulatory compulsion was removed (the pre-reform regime enforced annuitization through a 55\% tax penalty on alternative withdrawals; the post-reform regime is opt-in with marginal-rate tax neutrality) \citep{abi2020}; the \citet{goda2014} federal Thrift Savings Plan annuity option records an $\approx$86 pp household-level default-vs-opt-in gap, a quasi-experimental signal of the same order of magnitude. The Model 2 transport is the paper's primary quantification of this demand-side margin. Demand-side architecture is quantitatively comparable to the largest supply-side reforms considered here, while requiring no change to insurer pricing or product design.

\input{../tables/tex/cev_counterfactuals.tex}

\paragraph{Welfare under reform.} Under baseline pricing, the welfare gain from market access is small at every wealth-health cell---\cevGoodTwoHundredKBaseline{} for Good health with DFJ bequests at \$200{,}000---because most calibrated households would not optimally annuitize at current prices. Policy interventions that change the optimal choice generate the larger welfare gains. Group pricing (MWR $= 0.90$) roughly quadruples CEV at moderate wealth: \cevGoodTwoHundredKGroupPrice{} at \$200{,}000 (Good health, DFJ). The ``best feasible'' scenario combines group pricing with corrected beliefs and inflation protection, extending gains further: \cevGoodHundredKBestFeas{} at \$100{,}000 and \cevGoodTwoHundredKBestFeas{} at \$200{,}000. CEV is near zero at high wealth under baseline, group-pricing-only, and real-annuity-only scenarios. The values reported in the welfare table are a CRRA value-ratio welfare index. Because the model includes non-homothetic bequests and a consumption floor, these values should be read as directional access-value statistics rather than exact compensating variation; the ranking and signs across scenarios are preserved by the approximation.
...
\begin{tablenotes}
\small
\item Nine-channel structural baseline (rational + preferences + $\chi_{\text{LTC}} = \pChiLTC$). The Model 2 transport against the frictionless benchmark is reported separately (Section~\ref{sec:behavioral_results}).
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\paragraph{Grid convergence.} At the \pNQuad-node production quadrature rule, predicted ownership ranges from 25.6\% at the medium grid ($60 \times 20$) to 23.6\% at the fine grid ($100 \times 40$), bracketing the production grid (Table~\ref{A-tab:full_robustness_continued}). The coarse grid ($40 \times 15$) deviates more substantially (29.6\%) and is reported only as a diagnostic. The 2 pp variation across the production-and-finer range is small relative to the UK retention sensitivity range that drives the Model 2 bracket.
...
Under the baseline calibration ($\gamma = \pGamma$, MWR $= \pMwrBaseline$), the six standard rational channels predict \ownSixChannel{} ownership relative to a frictionless population benchmark of \ownFrictionless{}---above both empirical targets and echoing the residual overprediction in \citet{pashchenko2013}. The two preference channels bring the prediction to \ownEightChannelExt{}, and adding the structural public-care aversion channel yields the nine-channel Model 1 baseline of \ownNineChannelLTC{}, a slight underestimate of the two HRS empirical comparators ($\pctHRSIannPooled$ on the conventional any-annuity income proxy, 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$; $\pctHRSLifetime$ on the cleaner lifetime-contract indicator, 95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$). Model 2's UK reduced-form transport against the frictionless benchmark of \ownFrictionless{} delivers a complementary bracket of $[\ownBracketLow, \ownBracketHigh]$ with midpoint \ownHeadline{}: the conventional HRS income proxy falls inside the bracket and the lifetime-contract indicator sits just below the lower edge. The disciplined reading is the parallel report of Model 1 and Model 2, not a single moment-matched figure.

The exact Shapley decomposition provides the clearest attribution hierarchy. Among demand-suppressing channels in the eleven-channel extended specification, the at-purchase penalty (PED) is the largest contributor at literature magnitude, followed by pricing loads and the Reichling--Smetters health-mortality correlation; age-varying consumption needs, survival pessimism, and medical expenditure risk form the next tier. Source-dependent utility and Social Security enter as boosters with negative Shapley values. State-dependent utility, bequest motives, and public-care aversion are quantitatively negligible. Because the behavioral parameters $\lambda_W$ and $\psi_{\text{purchase}}$ are exploratory rather than moment-matched, the Shapley contributions for SDU and PED are illustrative of where literature-magnitude behavioral parameters land in the channel ranking, not identification of the magnitudes themselves.
...
The baseline MWR of \pMwrBaseline{} is drawn from \citet{mitchell1999} and \citet{wettstein2021}. Recent evidence suggests that annuity pricing has improved, with population-table MWRs of approximately 0.87 for age-65 immediate annuities in 2024 market conditions. At MWR $= 0.85$, the model predicts \ownMWREightyFive{} ownership (Table~\ref{tab:mwr_sensitivity}). Pricing remains the highest-leverage supply-side policy margin in this market.
...
This paper developed two complementary models of the annuity puzzle. Model 1 is a structural lifecycle decomposition. A nine-channel rational-plus-preference baseline predicts \ownNineChannelLTC{} US voluntary annuity ownership, a slight underestimate of the two HRS empirical comparators ($\pctHRSIannPooled$ on the conventional any-annuity income proxy, 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$; $\pctHRSLifetime$ on the cleaner lifetime-contract indicator, 95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$). This does not imply that behavioral factors do not matter. As an exploratory exercise, an eleven-channel extended specification layers two behavioral channels---source-dependent utility ($\lambda_W = \pLambdaW$) and a narrow-framing at-purchase penalty ($\psi_{\text{purchase}} = \pPsiPurchase$)---onto the baseline at literature-magnitude parameters. At these values each behavioral channel individually moves predicted ownership by more than almost any single rational, preference, or structural channel, but the two operate in opposite directions and largely offset. The eleven-channel prediction is \ownElevenChannelFull{} and is sensitive to assumptions---small shifts within the literature-defensible parameter range move predictions substantially in either direction---so the nine-channel structural baseline is the disciplined Model 1 reading.

Model 2 is a reduced-form transport that anchors a behavioral wedge externally on the UK 2015 pension-freedoms reform. The post/pre retention ratio (\pUKRetentionMid/\pUKRetentionPre{} at the mid anchor; range \pUKRetentionLow{}--\pUKRetentionHigh{}) is applied as a multiplicative wedge against the frictionless population benchmark of \ownFrictionless{}, yielding predicted US ownership of \ownHeadline{} with bracket $[\ownBracketLow, \ownBracketHigh]$. The conventional HRS income proxy lies inside the Model 2 bracket; the cleaner lifetime-contract indicator sits just below the lower edge. The two models reach the same neighborhood from independent directions: structural decomposition from inside the lifecycle problem, and cross-country transport from outside it.

The exact Shapley decomposition, computed over all $2^{11} = 2{,}048$ subsets of the eleven channels, provides the paper's preferred order-independent attribution. Among demand-suppressing channels, the at-purchase penalty and pricing loads are the two largest contributors, followed by the combined Med+R-S health-mortality channel, age-varying consumption needs, survival pessimism, and inflation erosion. Source-dependent utility and Social Security enter with negative Shapley values, as ownership boosters. The Shapley contributions for SDU and PED are illustrative of where literature-magnitude behavioral parameters land in the channel ranking rather than identification of the magnitudes themselves. Given that each behavioral channel individually exceeds nearly any single rational, preference, or structural channel in magnitude, and that the two largely offset, one would expect a meaningful amount of inconsistency in empirical and structural estimates of annuity demand across studies and over time---small differences in implicit behavioral assumptions can amplify, offset, or override otherwise clean variation in the channels typically identified.

Welfare and policy implications follow from the same calibration. Under current pricing, the welfare gain from annuity market access alone is small (under DFJ bequests, \cevGoodTwoHundredKDFJ{} at \$200{,}000 in Good health), because most calibrated households would not optimally buy at current prices. Larger welfare gains require policy reforms that change the optimal choice: group pricing roughly quadruples CEV at moderate wealth, and the best-feasible welfare package reaches \cevGoodTwoHundredKBestFeas{} at \$200{,}000. Two distinct policy levers operate on different margins: supply-side reforms (group pricing) raise the value of annuitization at the structural baseline; demand-side reforms (default architecture) operate on the choice-environment margin externally quantified by Model 2.
