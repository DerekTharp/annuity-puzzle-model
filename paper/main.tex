...
\begin{abstract}
\noindent
\citet{yaari1965} showed that a consumer with uncertain lifetime and no bequest motive should fully annuitize under actuarially fair pricing. Observed voluntary annuity ownership among US retirees is only a few percent. I build a lifecycle model that begins from a standard rational and preference-based account of annuity demand---six rational channels (Social Security, bequest motives, combined medical-expenditure risk and health-mortality correlation, survival pessimism, pricing loads, inflation erosion) and two preference channels (age-varying consumption needs, state-dependent utility)---and then \emph{adds two behavioral channels} that operate at distinct decision moments. Source-dependent utility (Force A; \citealp{blanchett2024,blanchett2025}) raises annuitization by converting portfolio wealth into ``spendable income.'' A narrow-framing purchase-event disutility (Force B; \citealp{barberishuang2009,tverskykahneman1992}) suppresses it through loss aversion over the unrecouped premium until breakeven. Without the two behavioral additions the model substantially overshoots observed US ownership; with them, the model dissolves the puzzle. The joint contribution of all ten channels is quantified using exact Shapley values computed over all 1{,}024 channel subsets.

\noindent
The Force B purchase-event disutility parameter $\psi_{\text{purchase}}$ is the only parameter calibrated through behavioral identification, and is anchored entirely to UK microdata from the 2015 pension-freedoms reform---the only natural experiment in which a population of retirees was directly observed shifting from mandatory to voluntary annuitization. From the English Longitudinal Study of Ageing (ELSA), $\pctELSAWaveSixAnnuity$ of wave 6 (2012--13) DC pension recipients reported annuity-style income under the pre-freedoms mandate; pooling waves 8--11 (2016--24), only $\pctELSAPostAnnuity$ of DC pot holders chose annuitization from a lump sum. The empirically defensible UK behavioral anchor range spans the conservative ABI aggregate (60 pp behavioral residual after stripping the simultaneous tax-rule change) and the aggressive ELSA microdata total drop (88 pp). Mapped through the model, this yields a bracket of predicted US voluntary ownership of $[\ownBracketLow, \ownBracketHigh]$. Two HRS measures of US lifetime annuity ownership are reported in parallel as out-of-sample empirical targets: $\pctHRSLifetime$ (95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$, the cleaner fat-file lifetime annuity contract indicator), and $\pctHRSIannPooled$ (95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$, the conventional any-annuity income proxy used in prior literature). Both are consistent with the model's UK-anchored bracket: the income-proxy measure sits comfortably inside it, and the cleaner lifetime indicator's confidence interval overlaps its lower edge. No US moment is used to discipline $\psi_{\text{purchase}}$; consistency with US ownership is an out-of-sample validation, not the calibration target.

\noindent
Under modern annuity pricing (MWR $= \pMwrBaseline$, \citealp{wettstein2021}), the model with all six rational channels active predicts \ownSixChannel{} ownership; adding the two preference channels (age-varying needs and state-dependent utility) lowers this to \ownEightChannelExt{}. Without behavioral additions, the model thus substantially overshoots both empirical targets. Activating Force A alone (source-dependent utility) raises predicted ownership to \ownNineChannelSDU{}; activating Force B (the UK-anchored purchase-event disutility) on top of Force A then yields the bracket $[\ownBracketLow, \ownBracketHigh]$. The two behavioral forces are individually large in magnitude but partly offset, and they enter the exact Shapley decomposition with opposite signs (\shapSDU{} vs.\ \shapNarrowFraming{} pp), confirming empirically distinct mechanisms rather than redundant parameterizations of the same wedge. Policy counterfactuals show that supply-side reform (group pricing) and demand-side reform (default annuitization) operate on distinct margins and are quantitatively comparable as policy levers.
...
The paper then \emph{adds two behavioral channels} that operate at distinct decision moments and point in opposite directions. Source-dependent utility (Force A) captures the post-purchase ``license to spend'' that retirees apply to guaranteed income flows but not portfolio assets \citep{shefrin1988, thaler1999, blanchett2024, blanchett2025}; its parameter $\lambda_W$ is set ex ante from the Blanchett--Finke spending differential. A narrow-framing purchase-event disutility (Force B) captures the at-purchase loss aversion over the unrecouped premium until breakeven \citep{tverskykahneman1992, barberishuang2009, brown2008framing, chalmersreuter2012}; its parameter $\psi_{\text{purchase}}$ is the only behaviorally-identified parameter in the model. \textbf{$\psi_{\text{purchase}}$ is calibrated entirely from UK microdata}, drawing on the 2015 pension-freedoms reform---the only natural experiment in which a population of retirees was directly observed shifting from mandatory to voluntary annuitization. The English Longitudinal Study of Ageing (ELSA) records this shift household-by-household: $\pctELSAWaveSixAnnuity$ of wave 6 (2012--13) DC pension recipients reported annuity-style income under the pre-freedoms mandate, falling to $\pctELSAPostAnnuity$ choosing annuitization from a lump sum once the mandate was removed (waves 8--11, $n=\nELSAPostLumpSum$). No US moment is used to discipline either behavioral parameter; consistency with US ownership is an out-of-sample validation, not the calibration target.

The paper's contribution is fourfold. First, the unified framework---the first to nest age-varying consumption needs alongside the established rational and preference channels of the annuity-puzzle literature. Second, the formal incorporation of two countervailing behavioral channels, each calibrated from independent empirical evidence, that operate at distinct decision moments. Third, the use of the UK 2015 pension-freedoms reform as an out-of-sample identification source for the purchase-event disutility, drawing on individual-level ELSA data rather than industry aggregates. Fourth, an exact Shapley decomposition over all $1{,}024$ channel subsets that attributes the demand reduction without order dependence.

The results have a three-layer structure. First, under modern annuity pricing (MWR $= \pMwrBaseline$, \citealp{wettstein2021}), the six rational channels alone predict ownership of \ownSixChannel{} relative to a frictionless population benchmark of \ownFrictionless{}; adding the two preference channels brings the prediction to \ownEightChannelExt{}. The standard rational and preference account therefore substantially overshoots the empirical targets---echoing the residual gap in \citet{pashchenko2013} and \citet{peijnenburg2016}. Second, adding Force A alone (source-dependent utility) raises predicted ownership to \ownNineChannelSDU{} by unlocking the post-purchase ``license to spend'' on annuity income---confirming that one behavioral channel without the other is also insufficient and points the wrong way. Third, adding the UK-anchored Force B on top of Force A yields the headline bracket of $[\ownBracketLow, \ownBracketHigh]$ across the empirically defensible UK behavioral anchor range. Both HRS measures of US lifetime annuity ownership are consistent with this bracket: $\pctHRSIannPooled$ (95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$, the conventional any-annuity income proxy) sits comfortably inside it, and $\pctHRSLifetime$ (95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$, the cleaner lifetime contract indicator) overlaps its lower edge. The model dissolves the puzzle robustly to the empirical-target choice. The two behavioral forces are individually large but partly offset, and neither parameter is tuned to a US moment: $\lambda_W$ comes from \citet{blanchett2025}'s spending differential, and $\psi_{\text{purchase}}$ comes from UK ELSA microdata.
...
The policy and welfare results extend beyond the ownership match. Group annuity pricing (MWR $= 0.90$) raises predicted ownership at the bracket midpoint to \ownGroupPricing{}, while removing the purchase-event disutility entirely (default architecture; $\psi_{\text{purchase}} = 0$) raises it to \ownNineChannelSDU{}---a default-vs-opt-in elasticity consistent with the UK 2015 reform and the \citet{goda2014} federal Thrift Savings Plan natural experiment, neither used in calibration. Social Security cut counterfactuals reveal that a 23\% benefit cut---the projected consequence of trust fund exhaustion around 2033---raises private annuity demand to \ownSSCutTwentyThree{}, but the largest cuts eventually reduce demand by removing the income floor that makes private annuitization feasible at all. Under current pricing the welfare gain from annuity market access is small across the wealth distribution because most calibrated households optimally do not annuitize, so access is approximately neutral; policy interventions that change the optimal choice (group pricing, default architecture) generate the larger welfare gains.
...
\paragraph{Out-of-sample test.} The calibrated $\psi_{\text{purchase}}$ bracket is set entirely from UK data, with no reference to observed US ownership. The resulting US predicted voluntary annuitization bracket is therefore an out-of-sample prediction. Section~\ref{sec:results} reports this prediction against both HRS measures of US lifetime annuity ownership.
...
\paragraph{Narrow-framing purchase-event disutility (Force B).} The Force B parameter $\psi_{\text{purchase}}$ is the only behaviorally-identified parameter in the model. It is calibrated by single-moment SMM to UK microdata from the 2015 pension-freedoms reform---the only natural experiment in which a population of retirees was directly observed shifting from mandatory to voluntary annuitization. The paper reports a \emph{calibration bracket} rather than a single production point, reflecting two independent UK evidence streams.
...
\emph{ELSA microdata (upper bound, $\psi_{\text{purchase}} = \pPsiBracketHigh$).} The English Longitudinal Study of Ageing records the disposition shift household-by-household. In wave 6 (2012--13, pre-freedoms), $\pctELSAWaveSixAnnuity$ of DC pension recipients reported annuity-style income under the mandate. Pooling waves 8--11 (2016--24, post-freedoms), only $\pctELSAPostAnnuity$ of DC pot holders chose annuitization from a lump sum (n=\nELSAPostLumpSum), implying a total drop of approximately 88 pp. The microdata-anchored calibration adopts this drop without rational stripping (since the ELSA cohort comparison conditions on having a DC pot at decision time and is therefore less sensitive to the aggregate sales-mix concerns in ABI data); it implies $\psi_{\text{purchase}} = \pPsiBracketHigh$ and predicted US ownership of \ownBracketLow{}.

\emph{Bracket interpretation.} The pair $\psi_{\text{purchase}} \in [\pPsiBracketLow, \pPsiBracketHigh]$ implies predicted US voluntary ownership in $[\ownBracketLow, \ownBracketHigh]$. The wider sensitivity range across alternative single-anchor SMM specifications (Appendix~\ref{A-app:psi_anchors}) extends from \ownBracketWideLow{} (above-bracket: ELSA total drop with no sample-weighting correction) to \ownBracketWideHigh{} (below-bracket: 55 pp behavioral). A key feature of this calibration approach: $\psi_{\text{purchase}}$ is identified entirely from non-US data. No US moment is used to discipline either bracket bound; consistency with US ownership is an out-of-sample validation, not the calibration target.
...
\paragraph{Population sample and empirical targets.} The wealth distribution is drawn from the RAND HRS longitudinal file, selecting single retirees aged 65--75 using filters comparable to \citet{lockwood2012}. The resulting sample contains 793 individuals. The main analysis restricts to the 566 individuals with liquid wealth above \pMinWealth, as those with negligible wealth cannot meaningfully annuitize. The structural model maps each household's state to a predicted decision rather than estimating moments from the sample distribution.
...
\item \emph{Any-annuity income proxy (\texttt{r\{w\}iann}; conventional measure).} The RAND HRS variable \texttt{r\{w\}iann} flags any positive annuity income in the wave, including DC pension withdrawals and short-period payouts not life-contingent. Pooled across waves 5--9, \pctHRSIannPooled{} of person-waves report positive annuity income (Wilson 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$). This measure is used by most prior literature (e.g., \citealp{lockwood2012}, who reported a 3.6\% rate for single retirees 65--69 using the same income variable). It overstates lifetime annuity ownership by conflating it with one-time DC pension withdrawals.
\end{enumerate}

The model's UK-anchored bracket of $[\ownBracketLow, \ownBracketHigh]$ predicted US ownership is robust to the empirical-target choice: the conventional any-annuity measure sits comfortably inside the bracket, and the cleaner lifetime contract indicator's confidence interval overlaps the bracket's lower edge. Section~\ref{sec:results} reports headline results against both measures.
...
Table~\ref{tab:retention} presents an intuitive sequential decomposition of predicted voluntary annuity ownership, adding the six standard rational channels one at a time. Because this exercise is order-dependent, I use it as a descriptive path through the model and treat the exact Shapley values below as the preferred attribution.
...
\paragraph{Bracket against the empirical targets.} Both HRS measures of US lifetime annuity ownership are consistent with the model's UK-anchored bracket. The conventional any-annuity income proxy ($\pctHRSIannPooled$, Wilson 95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$) sits comfortably inside the bracket. The cleaner lifetime contract indicator ($\pctHRSLifetime$, Wilson 95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$) overlaps the bracket's lower edge. Neither bracket bound is tuned to a US moment; consistency with both US measures is an out-of-sample validation. The model dissolves the puzzle robustly to the empirical-target choice.
...
The headline ten-channel prediction at the bracket midpoint sits within a 90\% confidence interval of [\mcLowCIOwnership, \mcHighCIOwnership] (median \mcMedianOwnership, IQR [\mcLowIQROwnership, \mcHighIQROwnership]) under joint parameter uncertainty. Both HRS empirical targets ($\pctHRSLifetime$ on the lifetime contract indicator, $\pctHRSIannPooled$ on the conventional any-annuity income proxy) lie within or overlap the resulting empirical bracket spanned by alternative model specifications. This robustness check shows that the predicted-versus-observed gap is sensitive primarily to the behavioral parameters, not to the rational and preference channels: under any plausible draw of the rational and preference parameters, the model's predicted ownership lies within a tight band around the bracket midpoint.
...
Under the bracket-midpoint calibration with all ten channels active, the welfare gain from annuity market access is small at every wealth level. The largest single-cell CEV is \cevGoodHundredKNoBq{} at \$100{,}000 in Good health under no bequests; with the DFJ bequest specification it is \cevGoodHundredKDFJ{} at the same wealth and health. CEV at \$200{,}000 in Good health is \cevGoodTwoHundredKNoBq{} (no bequest) and \cevGoodTwoHundredKDFJ{} (DFJ). At wealth above \$500{,}000 and at all health levels Fair or Poor, CEV is essentially zero. At the population level, mean CEV under DFJ bequests is \popMeanCevDFJ{}, with \popFracPosDFJ{} of households deriving any positive welfare gain and \popFracAboveOneDFJ{} deriving gains exceeding 1\% of consumption.

The economic content of these small CEVs is the same as the low predicted ownership rate: with the bracket-midpoint behavioral calibration in place, most households would not optimally purchase an annuity at current prices, so giving them access to the market provides little marginal value. The CEV grid is therefore the welfare counterpart of the ownership prediction---both confirm that under the bracket-midpoint calibration the marginal household is approximately indifferent at the no-purchase margin. Larger welfare gains appear only when policy changes the optimal choice: lowering the load (group pricing) or removing the narrow-framing penalty (default architecture), reported in Section~\ref{sec:counterfactuals}.
...
\paragraph{Pricing loads as a dominant supply-side lever.} Group annuity pricing (MWR $= 0.90$), achievable through employer plans or government-sponsored pools \citep{james2006}, raises predicted ownership from the bracket midpoint to \ownGroupPricing. A public option at MWR $= 0.95$ reaches \ownPublicOption, and an actuarially fair benchmark at MWR $= 1.00$ reaches \ownActuariallyFair. These results quantify the pricing channel's importance: a modest improvement in the money's worth ratio---from \pMwrBaseline{} to 0.90---raises participation by roughly nine percentage points relative to the bracket midpoint.
...
Under the baseline calibration ($\gamma = \pGamma$, MWR $= \pMwrBaseline$), the six standard rational channels predict \ownSixChannel{} ownership relative to a frictionless population benchmark of \ownFrictionless{}---substantially overshooting both empirical targets and echoing the residual overprediction in \citet{pashchenko2013}. The two preference channels bring the prediction to \ownEightChannelExt{}. The two behavioral channels then operate in opposite directions: source-dependent utility raises predicted ownership to \ownNineChannelSDU{}, and the UK-anchored narrow-framing purchase-event disutility produces the bracket $[\ownBracketLow, \ownBracketHigh]$. Both HRS measures of US ownership---$\pctHRSLifetime$ (95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$) and $\pctHRSIannPooled$ (95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$)---are consistent with this bracket. The paper is therefore strongest when read as a layered result: rational and preference channels alone substantially overshoot, and the two behavioral channels---each calibrated from independent external evidence---collectively complete the dissolution while introducing the policy-relevant default-vs-opt-in margin.
...
Behavioral channels enter the model through two distinct parameters operating at different decision moments. Source-dependent utility ($\lambda_W = \pLambdaW$) is calibrated to the Blanchett--Finke spending differential and is identical to the source-dependence parameter in a companion paper on Social Security claiming \citep[][under review]{tharpFPR2026}, providing cross-paper consistency: the calibration was set in the companion paper before this paper's predictions were computed. The narrow-framing purchase-event disutility ($\psi_{\text{purchase}}$) is calibrated entirely from UK microdata, with the bracket bounds spanning two independent UK evidence streams: $\psi_{\text{purchase}} = \pPsiBracketLow$ (ABI aggregate, 60 pp behavioral residual after stripping the rational tax-removal response) and $\psi_{\text{purchase}} = \pPsiBracketHigh$ (ELSA microdata, 88 pp total drop, household-level disposition data from waves 8--11 with $n=\nELSAPostLumpSum$). The corresponding US ownership predictions span $[\ownBracketLow, \ownBracketHigh]$, with both HRS empirical targets consistent with the bracket. Out-of-sample validation comes from a separate natural experiment not used in the calibration: the \citet{goda2014} federal Thrift Savings Plan default evidence is quantitatively consistent with the model's implied elasticity. \citet{huscott2007} showed that prospect theory loss aversion reduces optimal annuitization substantially; the narrow-framing channel formalizes this insight. \citet{ameriks2020} document the importance of long-term care aversion in late-life saving decisions, which remains to be incorporated. \citet{madrian2001} showed that defaults dominate retirement savings behavior more generally; in this paper that finding is operationalized through the demand-side counterfactual ($\psi_{\text{purchase}} = 0$) rather than as a separate channel.
...
This paper built a lifecycle model that begins from a standard rational and preference-based account of annuity demand and then \emph{adds two behavioral channels} grounded in independent empirical evidence. Without the behavioral additions, the model substantially overshoots both empirical targets; with them, the model dissolves the puzzle. Predicted US voluntary ownership lies in the bracket $[\ownBracketLow, \ownBracketHigh]$, anchored entirely to UK microdata. Both standard HRS measures of US lifetime annuity ownership are consistent with the bracket: $\pctHRSIannPooled$ (95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$, the conventional any-annuity income proxy) sits comfortably inside it; $\pctHRSLifetime$ (95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$, the cleaner lifetime contract indicator) overlaps the bracket's lower edge.

The result has a three-layer structure. Six standard rational channels predict \ownSixChannel{} ownership relative to a frictionless benchmark of \ownFrictionless{}. Two preference channels---age-varying consumption needs and state-dependent utility---bring this to \ownEightChannelExt{}, still substantially above both empirical targets. The two behavioral channels then operate in opposite directions: source-dependent utility (Force A, calibrated externally from \citet{blanchett2025}'s spending-differential evidence) raises predicted ownership to \ownNineChannelSDU{} by unlocking the post-purchase ``license to spend,'' and the narrow-framing purchase-event disutility (Force B, calibrated by single-moment SMM to UK ELSA microdata from the 2015 pension-freedoms reform) yields the bracket $[\ownBracketLow, \ownBracketHigh]$. Each layer is a substantive contribution, and the calibration is identified out of sample at every stage: no US moment is used to discipline either behavioral parameter, $\lambda_W$ is set ex ante from the Blanchett--Finke spending differential, and $\psi_{\text{purchase}}$ is calibrated entirely from the UK natural experiment.

The exact Shapley decomposition, computed over all $1{,}024$ channel subsets, provides the paper's preferred attribution. Among demand-suppressing channels, the narrow-framing purchase-event disutility (Force B) has the largest Shapley value, followed by pricing loads and the combined Med+R-S health-mortality channel; age-varying needs, survival pessimism, and inflation erosion form the next tier. Source-dependent utility (Force A) enters with a negative Shapley value (raising demand), reflecting the post-purchase ``license to spend'' on annuity income. The two behavioral channels enter with opposite-sign Shapley values, confirming that they operate at empirically distinct decision moments and are not redundant. Social Security also enters with a negative Shapley value, raising annuity demand in frictionless environments but crowding out private annuitization once market frictions are active.

The welfare and policy implications follow from the same calibration. Under current pricing, the welfare gain from annuity market access alone is small (under DFJ bequests, \cevGoodTwoHundredKDFJ{} at \$200{,}000 in Good health), because most calibrated households would not optimally buy at current prices. Larger welfare gains require policy reforms that change the optimal choice: group pricing roughly quadruples CEV at moderate wealth, and the best feasible package (group pricing combined with default architecture) reaches \cevGoodTwoHundredKBestFeas{} at \$200{,}000. Two distinct policy levers operate on different margins: supply-side reforms (group pricing) raise the value of annuitization; demand-side reforms (default architecture) remove its psychological cost.
