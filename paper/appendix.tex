...
The headline calibration bracket sets $\psi_{\text{purchase}} \in [\pPsiBracketLow, \pPsiBracketHigh]$, with the conservative bracket end ($\pPsiBracketLow$) anchored on the rational-corrected ABI behavioral residual (60 pp midpoint of the 55--65 pp range; \citealp{abi2020}) and the aggressive bracket end ($\pPsiBracketHigh$) anchored on the descriptive ELSA microdata total drop (88 pp, n=\nELSAPostLumpSum). The corresponding bracket of predicted US voluntary ownership is $[\ownBracketLow, \ownBracketHigh]$. This appendix reports the full sensitivity table across alternative single-moment SMM specifications. Two HRS measures of US lifetime annuity ownership are reported in parallel as out-of-sample empirical targets: $\pctHRSLifetime$ (95\% CI $[\pctHRSLifetimeCILow, \pctHRSLifetimeCIHigh]$, lifetime contract indicator) and $\pctHRSIannPooled$ (95\% CI $[\pctHRSIannCILow, \pctHRSIannCIHigh]$, conventional any-annuity income proxy). The income-proxy measure sits inside the bracket; the cleaner lifetime indicator's CI overlaps the bracket's lower edge.
...
\midrule
\textbf{C}$_{\text{low}}$  & ABI behavioral residual low (55 pp)  & \pPsiUKLow  & \ownPsiUKLow  & Tax-stripped (ABI aggregate) \\
\textbf{C}$_{\text{mid}}$  & ABI behavioral residual mid (60 pp)  & \textbf{\pPsiUKMid}  & \textbf{\ownPsiUKMid}  & \textbf{Conservative bracket end (ABI)} \\
\textbf{C}$_{\text{high}}$ & ABI behavioral residual high (65 pp) & \pPsiUKHigh & \ownPsiUKHigh & Tax-stripped (ABI aggregate) \\
...
\begin{tablenotes}
\small
\item The headline calibration bracket is $\psi_{\text{purchase}} \in [\pPsiBracketLow, \pPsiBracketHigh]$, with the conservative bracket end (C-mid, ABI aggregate after stripping the rational tax-removal response) and the aggressive bracket end (ELSA microdata, raw 88 pp pre/post drop with no tax stripping). The corresponding bracket of predicted US voluntary ownership is $[\ownBracketLow, \ownBracketHigh]$. Values shown for additional anchors are computed from the same single-moment SMM under alternative identifying targets and reported as a sensitivity range.
...
Table~\ref{tab:pashchenko} evaluates her channel set within the present framework. This comparison differs from a replication in three respects: the present model uses DFJ luxury-good bequests rather than the homothetic specification she employed; a continuous annuitization fraction rather than a binary purchase decision; and no housing illiquidity. The six-channel rational model under the present 10-channel reformulation predicts \ownSixChannel, in the neighborhood of her approximately 20\% finding, confirming that standard rational channels explain substantial but not all of the Yaari-to-observed gap. Adding the channels Pashchenko omitted (the combined Med+R-S health-mortality correlation, survival pessimism, age-varying needs, and inflation erosion) reduces predicted ownership further to \ownEightChannelExt{}, but a substantial residual remains: the eight rational + preference channels still substantially overshoot both empirical targets. Closing the residual to the empirical bracket requires the two behavioral additions documented in the body (Force A: source-dependent utility; Force B: UK-disciplined narrow-framing purchase-event disutility); see Section~\ref{M-sec:behavioral_results}.
...
\textbf{\citet{pashchenko2013}.} The Pashchenko comparison is reported as Table~\ref{tab:pashchenko}. The six-channel rational model predicts \ownSixChannel{} ownership, close to her reported $\approx$20\%, confirming that standard rational channels alone leave a substantial residual gap relative to observed ownership. The channels Pashchenko omits (the combined Med+R-S correlation, survival pessimism, age-varying needs, and inflation erosion) reduce the residual but do not close it on their own; the residual is closed only after the two behavioral additions documented in the body (Section~\ref{M-sec:behavioral_results}).
