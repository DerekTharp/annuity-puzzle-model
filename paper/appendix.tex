...
Panel A shows that grid convergence is tight: ownership ranges from \gridConvProd{} at the production $80 \times 30$ grid to \gridConvVeryFine{} at $120 \times 50$, with the change between the two finest grids ($100 \times 40$ to $120 \times 50$) being \gridConvFineStep{} pp. The production resolution is biased downward by approximately \gridConvBias{} pp relative to the most refined grid. Panel B shows that quadrature convergence improves steadily with node count: ownership at $n_{\text{quad}} \in \{9, 11, 13, 15\}$ is \quadNine, \quadEleven, \quadThirteen, and \quadFifteen, rising within a \quadBand{} pp band. Lower-order rules ($n_{\text{quad}} \leq 7$) deviate by up to 2 pp because the lognormal medical expense distribution ($\sigma \approx 1.4$) has heavy right tails that interact with the Medicaid floor; insufficient tail resolution at low orders shifts marginal households across the discrete ownership threshold ($\alpha^* > 0$).

Nine-node Gauss--Hermite is adopted as the production standard as a balance between accuracy and computational cost. The reference specification (Panel C: $120 \times 50$ grid, 11-node GH) gives \gridConvRef{}, about \gridConvTotalUnc{} pp above the production point. The total numerical uncertainty---combining grid coarseness and quadrature error---is therefore approximately \gridConvTotalUnc{} pp on this mean-SS diagnostic. The qualitative ranking of channels is stable across all grid and quadrature specifications tested, which is the property the paper's conclusions rely on.
...
Table~\ref{tab:grid_convergence} (Panel B) documents the convergence: binary ownership at the production grid moves from \quadThree{} at $n_{\text{quad}} = 3$ to \quadNine{} at $n_{\text{quad}} = 9$ and rises within a \quadBand{} pp window thereafter (\quadNine, \quadEleven, \quadThirteen, \quadFifteen{} at $n_{\text{quad}} \in \{9, 11, 13, 15\}$). Lower-order rules deviate by up to 2 pp because of the tail--floor interaction: small changes in the quadrature approximation of the medical-expense tail shift marginal households across the discrete ownership threshold. Nine nodes balances accuracy against computational cost; we adopt it as the production standard.
...
Table~\ref{tab:euler} reports summary statistics. The median residual is zero: most interior grid points satisfy the Euler equation exactly. The mean residual is \eulerMeanPct, with approximately \eulerPctAboveOne{} of grid points exceeding 1\% and \eulerPctAboveFive{} exceeding 5\%. The large maximum residuals ($\approx 0.99$) occur at low-wealth states where the finite-difference derivative of the piecewise-linear value function is a poor approximation to the true marginal value. Grid convergence is visible: the fraction of points above 1\% declines from \eulerPctOneCoarse{} at $40 \times 15$ to \eulerPctOneFine{} at $100 \times 40$.
