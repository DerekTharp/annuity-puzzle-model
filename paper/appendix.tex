...
Within-model sensitivity is reported over the ranges $\lambda_W \in \{0.5, 0.625, 0.75, 0.85\}$ and $\psi_{\text{purchase}} \in \{0.01, 0.05, 0.09\}$. These ranges are stated for transparency about the exploratory status of the parameters: any single combination within the grid is consistent with the available behavioral evidence at the literature's order of magnitude. The body of the paper does not rely on a specific point value of either parameter for its headline result. The disciplined Model 1 reading is the nine-channel structural baseline of \ownNineChannelLTC{}. Model 2 reports the reduced-form UK transport in parallel: the frictionless population benchmark of \ownFrictionless{} is multiplied by the proportional retention factor $\rho = $ post/pre $\approx 17/95$ to yield the production midpoint of \ownHeadline{} with sensitivity bracket $[\ownBracketLow, \ownBracketHigh]$ across UK retention anchors. The two models are reported in parallel as complementary readings, not as a single headline path.
