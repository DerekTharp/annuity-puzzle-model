...
Medium & $60 \times 20$ & 9 & 20.3 & 0.0323 \\
\textbf{Production} & $\mathbf{80 \times 30}$ & \textbf{9} & \textbf{21.4} & \textbf{0.0336} \\
Fine & $100 \times 40$ & 9 & 20.7 & 0.0319 \\
Very fine & $120 \times 50$ & 9 & 20.7 & 0.0317 \\
\midrule
\multicolumn{5}{l}{\textit{Panel B: Quadrature convergence ($80 \times 30$ grid)}} \\
 & $80 \times 30$ & 3 & 24.6 & 0.0332 \\
 & $80 \times 30$ & 5 & 27.9 & 0.0376 \\
 & $80 \times 30$ & 7 & 18.4 & 0.0250 \\
 & $80 \times 30$ & \textbf{9} & \textbf{21.4} & \textbf{0.0336} \\
 & $80 \times 30$ & 11 & 17.8 & 0.0264 \\
\midrule
\multicolumn{5}{l}{\textit{Panel C: Reference}} \\
 & $120 \times 50$ & 11 & 16.8 & 0.0252 \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
Panel A shows that the grid is well-converged at the production resolution: ownership varies by only 1.1 percentage points across the four grids tested, and mean $\bar{\alpha}$ is stable within 0.002. Panel B reveals that quadrature convergence is the binding numerical concern. Binary ownership oscillates across node counts because the lognormal medical expense distribution ($\sigma \approx 1.4$) has heavy right tails that interact with the Medicaid floor: small shifts in the quadrature approximation of this tail push marginal households above or below the discrete ownership threshold ($\alpha^* > 0$). The continuous metric $\bar{\alpha}$ is more stable but still exhibits some sensitivity.

The production standard is 9-node Gauss--Hermite, which is well-converged across grids (Panel A, ownership range 20.3--21.4\%) and sits centrally among the quadrature variants in Panel B. The reference specification (Panel C: $120 \times 50$ grid, 11-node GH) gives 16.8\%, about 4.6 pp below the production result. This numerical uncertainty is modest relative to the Shapley channel contributions (the smallest substantive channel has a Shapley value of 2.2 pp). The qualitative ranking of channels is stable across all grid and quadrature specifications tested.
...
Table~\ref{tab:grid_convergence} (Panel B) documents this: binary ownership oscillates between 18\% and 28\% as nodes increase from 3 to 11. The continuous metric $\bar{\alpha}$ is more stable but still varies. This oscillation arises because small changes in $\alpha^*$ near the ownership threshold shift marginal households across the binary cutoff. The 9-node rule is well-converged across grid resolutions (Panel A) and is adopted as the production standard.
