...
Because the participation level is sensitive to $\psi$, a natural question is whether the channel ranking is too. It is not. Recomputing the full nine-channel Shapley game at the focal value $\psi = 0.981$ (Table~\ref{tab:shapley_psi981}) leaves pricing loads the dominant demand suppressor and bequest motives mid-pack---the two ranking claims the paper relies on. Survival pessimism's own Shapley contribution shrinks at the focal value, as expected when beliefs are made less pessimistic, but it remains the second-largest suppressor (\psiFocalPessimismAbs{} versus \psiFocalNextAbs~pp over age-varying needs, the next channel); the combined Med+R-S channel sits below both. The decomposition is computed on the $60 \times 20 \times 101$ grid, which the convergence table places within about one percentage point of production (Appendix~\ref{app:grid_convergence}); the level is not the object here.
