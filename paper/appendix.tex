...
Mean OOP, age 100, Fair health & \medMeanHundred & \$29,700 \\
95th pctile, age 100, Fair health & \medPctNinetyFive & \$111,200 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:bequest_recal} reports a forward simulation test at $\gamma = 2.0$ and $\gamma = \pGamma$ using the original $\theta = \pThetaDFJ$. At the baseline $\gamma = \pGamma$ the bequest-to-wealth ratio diverges modestly (Table~\ref{tab:bequest_recal}), below the threshold that would justify the strong assumption embedded in the recalibration formula. All results in the paper use Lockwood's original $\theta = \pThetaDFJ$ at all $\gamma$ values, treating the bequest parameters as fixed empirical estimates. This approach is conservative: it strengthens bequests relative to the recalibrated specification (whose higher $\theta$ at $\gamma = \pGamma$ weakens bequest utility through the higher CRRA exponent), making the predicted ownership results harder to achieve.
