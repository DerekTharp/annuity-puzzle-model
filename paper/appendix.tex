...
Baseline survival probabilities use the SSA administrative period life table employed by \citet{lockwood2012}. The cumulative death probabilities at each age from 65 to 110 are taken directly from Lockwood's replication code (BAP\_wtp.m); the table is annual, so no interpolation across ages is required.

Health-adjusted survival follows
\[
s(t, H) = s_{\text{base}}(t)^{\mu(H)},
\]
where $\mu(H)$ is the health-specific hazard multiplier. The baseline uses constant multipliers $[\pHazardGood, \pHazardFair, \pHazardPoor]$ for Good, Fair, and Poor health. These are informed by age-band estimates from the RAND HRS longitudinal file:
...
\citet{odeasturrock2023} estimate that individuals aged 65--69 underestimate survival to 75 by about 15 percentage points; their ten-year ratio gives the focal per-year factor $\psi = (0.71/0.86)^{1/10} \approx 0.981$. The direction of survival miscalibration is age-dependent: \citet{heimer2019} document a crossover near ages 65--70, after which individuals tend to \emph{overestimate} survival, so for this cohort mild optimism ($\psi$ at or above one) is also plausible. The paper therefore reports $\psi$ over the range $[\pPessimism, 1.0]$---a strong-pessimism end (at which subjective 10-year survival is $\psi^{10} \approx 0.66$ times objective), the O'Dea--Sturrock focal value, and objective beliefs---and retains $\psi = \pPessimism$ as the computational anchor. The participation level is sensitive to this choice (Section~\ref{M-sec:robustness}); the channel ranking is not (Appendix~\ref{app:psi_ranking}).

\subsection{Channel ranking at the focal survival-belief value}
\label{app:psi_ranking}

Because the participation level is sensitive to $\psi$, a natural question is whether the channel ranking is too. It is not. Recomputing the full nine-channel Shapley game at the focal value $\psi = 0.981$ (Table~\ref{tab:shapley_psi981}) leaves pricing loads the dominant demand suppressor and bequest motives mid-pack---the two ranking claims the paper relies on. Survival pessimism's own Shapley contribution falls to the third tier at the focal value, as expected when beliefs are made less pessimistic, and the combined Med+R-S channel takes the second tier. This lower-tier reshuffling is the same one documented across risk aversion in Section~\ref{M-sec:results}; it does not touch the dominant-channel result. The decomposition is computed on the $60 \times 20 \times 101$ grid, which the convergence table places within about one percentage point of production (Appendix~\ref{app:grid_convergence}); the level is not the object here. At that grid, pessimism's third-tier position at the focal value carries a wider margin over the next channel (5.1 versus 4.8~pp) than at coarser resolutions.

\input{../tables/tex/shapley_psi981.tex}
...
Deferred income annuities (DIAs) begin payments at a future age, typically 80 or 85. By concentrating payments in late life---when longevity risk is greatest---they offer higher per-dollar payouts. A DIA purchased at 65 with payments beginning at 80 pays roughly 3.3 times the per-period income of an immediate annuity (SPIA) per dollar of premium, and a DIA-85 roughly 7.5 times as much, at the exhibit's nominal pricing (the corresponding real-rate multiples are 2.7 and 5.6).
...
The joint stress test in Section~\ref{M-sec:joint_uncertainty} draws \nMCDraws{} parameter vectors from independent uniform distributions: the Poor-health hazard multiplier $\mu_P \sim U(2.5, 5.0)$, inflation $\pi \sim U(0.015, 0.025)$, MWR $\sim U(0.83, 0.91)$, survival beliefs $\psi \sim U(0.92, 1.00)$ (deliberately wider than the reported $[\pPessimism, 1.0]$ calibration range, extending the stress test mildly beyond the strong-pessimism end), and the consumption-needs slope $\delta_c \sim U(0.01, 0.03)$. Risk aversion is held fixed at the baseline. Each range is symmetric around its production value (e.g., $\psi$ around $\pPessimism$; $\mu_P$ around $\pHazardPoor$), so the central calibration is the central tendency of the draw. For each draw the model is solved on a convergence-verified $60 \times 20 \times 51$ grid, coarser than production for tractability across \nMCDraws{} draws.
...
%% ================================================================
\section{Sex-Blended Mortality Robustness}
\label{app:blended_mortality}

The headline model prices the annuity and evaluates survival off a single life table matching the male column of the SSA 2003 period table, while the model-eligible sample is \blendedFemaleShare{} female (Section~\ref{M-sec:calibration}). Table~\ref{tab:shapley_blended} recomputes the full nine-channel game with the life table replaced by the survivorship mixture of the SSA 2003 male and female period tables at the sample's female share, used for both survival and pricing, preserving the headline's same-table convention; the replication package documents the construction. The ranking is unchanged: pricing loads contribute \blendedLoads~pp, within rounding of the headline attribution, and bequest motives remain mid-pack (\blendedBequests~pp). The ownership level falls to \blendedOwn{} because the longer-lived blended population faces a lower fair payout at the same premium---one more instance of the level moving while the ranking does not.

\input{../tables/tex/shapley_blended_mortality.tex}
...
Display items referenced from the main text: the \citet{pashchenko2013} channel comparison, the joint parameter-uncertainty summary, the lifecycle moment check, the cross-sectional ownership logit, the Social Security cut schedule, the welfare counterfactual CEVs, and the risk-aversion, hazard-multiplier, policy-function, and welfare-surface figures.
