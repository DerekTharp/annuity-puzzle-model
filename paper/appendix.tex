...
Table~\ref{tab:grid_convergence} (Panel B) documents the convergence: binary ownership at the production grid moves from \quadThree{} at $n_{\text{quad}} = 3$ to \quadNine{} at $n_{\text{quad}} = 9$ and rises within a \quadBand{} pp window thereafter (\quadNine, \quadEleven, \quadThirteen, \quadFifteen{} at $n_{\text{quad}} \in \{9, 11, 13, 15\}$). Lower-order rules deviate by up to 2.4 pp because of the tail--floor interaction: small changes in the quadrature approximation of the medical-expense tail shift marginal households across the discrete ownership threshold. Nine nodes balances accuracy against computational cost; we adopt it as the production standard.
